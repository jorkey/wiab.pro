...
public class AttachmentDataDocHelper {
  ...
  /**
   * Creates an attachment data doc for a given attachment within a wavelet.
   *
   * @param wavelet the wavelet within which to create the data document.
   * @param attachmentId the id of the attachment.
   * @param attachmentUrl the URL of attachment.
   * @param contentType the MIME type of attachment.
   */
  public static void createAttachmentDataDoc(Wavelet wavelet,
      String attachmentId, String attachmentUrl, String contentType) {
    String attachmentDataDocId = dataDocIdFromAttachmentId(attachmentId);
    XmlStringBuilder xml = XmlStringBuilder.createEmpty();
    xml.appendText("<attachment_url value=\"" + attachmentUrl + "\"/>");
    xml.appendText("<mime_type value=\"" + contentType + "\"/>");
    wavelet.getDocument(attachmentDataDocId).appendXml(xml);
  }
}
