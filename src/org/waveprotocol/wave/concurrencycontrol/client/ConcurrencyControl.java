...
public class ConcurrencyControl implements ServerConnectionListener {
  ...
  private final UnsavedDataListener.UnsavedDataInfo unsavedDataInfo =
      new UnsavedDataListener.UnsavedDataInfo() {
        ...
        public String toString() {
          return "UnsavedDataListener.UnsavedDataInfo(" +
              "inFlightSize()=" + inFlightSize() + ", " +
              "estimateUnacknowledgedSize()=" + estimateUnacknowledgedSize() + ", " +
              "estimateUncommittedSize()=" + estimateUncommittedSize() + ", " +
              "lastAckVersion()=" + lastAckVersion() + ", " +
  ...
  public void close() {
    ...
    if (!clientOperationQueue.isEmpty()) {
      logger.error().log("Concurrency control closed with pending operations. "
          + "Data has been lost. clientOperationQueue.estimateSize()="
          + clientOperationQueue.estimateSize());
    }
  }

  @Override
  public void onOpen(HashedVersion connectVersion, HashedVersion currentVersion,
      List<WaveletOperation> operations, HashedVersion unacknowledgedDeltaVersion)
      throws TransformException, OperationException, ChannelException {
    ...
    if (unacknowledged != null) {
      if (unacknowledgedDeltaVersion != null) {
        AckInfo ack = new AckInfo(unacknowledged.size(), unacknowledgedDeltaVersion);
        acks.add(ack);
        triggerUnsavedDataListener();
      } else {
        mergeToClientQueue(0);
      }
      unacknowledged = null;
    }

    if (operations != null) {
      transformOperationsAndNotify(operations, connectVersion.getVersion(), currentVersion);
    }
   ...
   */
  private void transformOperationsAndNotify(List<WaveletOperation> operations,
      long appliedAtVersion, HashedVersion resultingVersion) throws TransformException, OperationException {
    // Sanity check
    ...
    if (appliedAtVersion < latestVersion) {
      throw new OperationException("Server sent a delta containing a version that is older than " +
          "the version at end of inferred server path." + this);
    }

    // Clear inferred server path as we can not recover past a server Delta.
    inferredServerPath.clear();
    startSignature = resultingVersion;

    // Transform against any unacknowledged ops.
    List<WaveletOperation> transformedServerDelta = operations;

    if (unacknowledged != null) {
      if (appliedAtVersion != unacknowledged.getTargetVersion().getVersion()) {
        throw new TransformException(
            "Cannot accept server version newer than unacknowledged. server version:"
                + appliedAtVersion + " unacknowledged version:"
                + unacknowledged.getTargetVersion() + this);
      }

      DeltaPair transformedPair = (new DeltaPair(unacknowledged, operations)).transform();
      ...
      unacknowledged = new WaveletDelta(unacknowledged.getAuthor(),
          resultingVersion, transformedPair.getClient());
    }
  ...
  public void onSuccess(int opsApplied, HashedVersion signature) throws TransformException {
    if (logger.isModuleEnabled() && logger.trace().shouldLog()) {
      logger.trace().log("onSuccess: version =", signature.getVersion());
    }
  ...
  private void makeFakeServerOpsFromAckAndNotify(HashedVersion ackedSignature)
    ...
    while (opItr.hasNext()) {
      ...
      versionOps.add(op.createVersionUpdateOp(ackedSignature.getVersion(), ackedSignature));
    }
  ...
  public void onServerDeltas(List<TransformedWaveletDelta> deltas) throws TransformException,
    ...
    for (TransformedWaveletDelta delta : deltas) {
      transformOperationsAndNotify(delta, delta.getAppliedAtVersion(), delta.getResultingVersion());
    }
