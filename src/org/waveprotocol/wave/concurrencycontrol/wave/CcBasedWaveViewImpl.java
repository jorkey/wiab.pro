...
public final class CcBasedWaveViewImpl implements CcBasedWaveView {
  ...
  public interface DisconnectedHandler {
     ...
     */
    void onWaveDisconnected(ReturnStatus detail);
  }
   ...
   */
  private static class MuxListener implements OperationChannelMultiplexer.ChannelsPresenceListener {
    /**
  ...
  public static CcBasedWaveView create(CcDocumentFactory<?> docFactory, SchemaProvider schemas,
      WaveId waveId, ParticipantId userId, final OperationChannelMultiplexer pipe, 
      final ViewChannel viewChannel, IdGenerator idGenerator, final LoggerBundle logger,
    ...
    FailureHandler failureHandler =
        new FailureHandler() {
          ...
          public void onWaveletFailed(OperationException failure) {
            ...
            new ReturnStatus(failure.isSchemaViolation()
                ? ReturnCode.SCHEMA_VIOLATION : ReturnCode.INVALID_OPERATION,
    ...
    final CcBasedWavelet.Factory waveletFactory = new CcBasedWavelet.Factory(
        waveId, contextFactory, participationHelper, docFactory, failureHandler, tapsFactory);

    final LocalWaveletHolder holder = new LocalWaveletHolder();
    WaveletFactory<CcBasedWavelet> factory =
        new WaveletFactory<CcBasedWavelet>() {
    ...
    final WaveViewImpl<CcBasedWavelet> view = WaveViewImpl.create(
        factory, waveId, idGenerator, userId, configurator);

    final MuxListener.WaveletListener waveletListener =
        new MuxListener.WaveletListener() {
    ...
    return new CcBasedWaveViewImpl(
        pipe, viewChannel, view, muxListenerFactory, accessibilityTracker);
  }

  /**
   * Creates a wave view (dependency injection constructor).
   */
  private CcBasedWaveViewImpl(OperationChannelMultiplexer pipe, ViewChannel viewChannel,
  ...
  @Override
  public void open(final OpenListener openListener, Collection<KnownWavelet> knownWavelets,
      Map<WaveletId, Set<SegmentId>> knownSegmentIds) {
    pipe.setChannelsPresenceListener(muxListenerFactory.create(openListener));
    pipe.open(knownWavelets, knownSegmentIds, new OperationChannelMultiplexer.StreamListener() {
  ...
  @Override
  public WaveletId getRootId() {
    return view.getRootId();
  }
