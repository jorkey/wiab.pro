...
public interface AnnotationBehaviour {
  ...
  public enum BiasDirection {
    /** A)  B -- default = associated with the previous character */
    LEFT,
  ...
  public enum InheritDirection {
    ...
    INSIDE,
    /** inherit the annotation from the other side to which the cursor is biassed. */
    OUTSIDE,
  ...
  public enum AnnotationFamily {
    /** Part of the core document model - e.g. font color or links. */
    CONTENT,
   ...
   */
  BiasDirection getBias(
      StringMap<Object> left, StringMap<Object> right, CursorDirection cursor);
   ...
   * @return Desired inheritance behaviour.
   */
  InheritDirection replace(
      StringMap<Object> inside, StringMap<Object> outside, ContentType type);

  /**
   * Defines the default model behaviour for annotations - that is, does not specify a bias,
   * replacement always comes from inside the cursor, and is prioritized behind elements.
   */
  public static class DefaultAnnotationBehaviour implements AnnotationBehaviour {
    ...
    @Override
    public InheritDirection replace(
        StringMap<Object> inside, StringMap<Object> outside, ContentType type) {
      // Default behaviour: inherit, unless rich text in which case leave formatting as-is
      return type == ContentType.RICH_TEXT ? InheritDirection.NEITHER :
          InheritDirection.INSIDE;
    }
