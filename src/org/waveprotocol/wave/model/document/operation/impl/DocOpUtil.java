...
public class DocOpUtil {
  ...
  public static DocInitialization asInitialization(final DocOp op) {
    if (op instanceof DocInitialization) {
      ...
      return new AbstractBufferedDocInitialization() {
        ...
        @Override
        public void setContext(WaveletOperationContext opContext) {
          op.setContext(opContext);
        }
  ...
  public static String toConciseString(DocOp op) {
    ...
    op.apply(createConciseStringBuilder(b));
    return b.toString();
  }

  public static DocOpCursor createConciseStringBuilder(final StringBuilder b) {
    return new DocOpCursor() {

      @Override
      public void deleteCharacters(String chars) {
        b.append("--").append(literalString(chars)).append("; ");
      }
      ...
      public void deleteElementStart(String type, Attributes attrs) {
        b.append("x< ").append(type).append(" ").append(toConciseString(attrs)).append("; ");
      }
      
      @Override
      public void replaceAttributes(Attributes oldAttrs, Attributes newAttrs) {
        b.append("r@ ").append(toConciseString(oldAttrs)).append(" ")
            .append(toConciseString(newAttrs)).append("; ");
      }
      
      @Override
      public void retain(int distance) {
        b.append("__").append(distance).append("; ");
      }

      @Override
      public void updateAttributes(AttributesUpdate attrUpdate) {
        b.append("u@ ").append(toConciseString(attrUpdate)).append("; ");
      }
      
      @Override
      public void annotationBoundary(AnnotationBoundaryMap map) {
        b.append("|| ").append(toConciseString(map)).append("; ");
      }
      ...
      public void elementStart(String type, Attributes attrs) {
        b.append("<< ").append(type).append(" ").append(toConciseString(attrs)).append("; ");
      }
    };
  }
        ...
        public void annotationBoundary(AnnotationBoundaryMap map) {
          ...
          for (Map.Entry<String, String> entry : changes.entrySet()) {
            if (entry.getValue() != null) {
              b.append(" \"")
                  .append(xmlTextEscape(annotationEscape(entry.getKey()) ))
                  .append("\"");
              b.append("=");
              b.append("\"")
                  .append(xmlTextEscape(annotationEscape(entry.getValue()) ))
                  .append("\"");
            } else {
  ...
  public static String[] visualiseOpWithDocument(final DocInitialization doc, final DocOp op) {
    ...
    final DocOpCursor opStringifier = createConciseStringBuilder(opB);
  ...
  /**
   * @return true, if the document operation first for document.
   */
  public static boolean isFirst(DocOp op) {
    return op.size() >= 1 &&
      DocOpComponentType.ELEMENT_START.equals(op.getType(0)) &&
      LineContainers.topLevelContainerTagname().equals(op.getElementStartTag(0));
  }
  
  /**
   * @return true, if the document operation deletes all content of the document
   *
   * // TODO: optimize by speed
   */
  public static boolean doesDeleteAllContent(DocOp op) {
    for (int i = 0; i < op.size(); i++) {
      if (DocOpComponentType.RETAIN.equals(op.getType(i))) {
        break;
      }
      if (DocOpComponentType.DELETE_ELEMENT_START.equals(op.getType(i))) {
        return true;
      }
    }
    return false;
  ...
  /**
   * @return true, if the document operation deletes reply in the document
   *
   * //TODO: optimize by speed
   */
  public static boolean doesDeleteReply(DocOp op) {
    for (int i = 0; i < op.size(); i++) {
      if (DocOpComponentType.DELETE_ELEMENT_START.equals(op.getType(i)) &&
          Blips.THREAD_INLINE_ANCHOR_TAGNAME.equals(op.getDeleteElementStartTag(i)) ) {
        return true;
      }
    }
    return false;
