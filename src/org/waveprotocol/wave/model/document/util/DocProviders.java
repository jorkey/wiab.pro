...
import org.waveprotocol.wave.model.document.DocumentProvider;
...
public class DocProviders {
   ...
   * implementation.
   */
  public final static DomDocument.Provider<DomDocumentImpl> ROJO =
      DomDocumentProviderImpl.create(DomDocumentImpl.BUILDER);

  /**
   * IndexedDocumentProvider with a substrate based on the "Rojo" dom
   * implementation
   */
  public final static IndexedDocProvider<Node, Element, Text, DomDocumentImpl> POJO =
   ...
   */
  public static <N, E extends N> OperationSequencer<Nindo> createCopyingSequencer(
      IndexedDocument<N, E, ? extends N> doc) {
    final IndexedDocument<Node, Element, Text> copy = POJO.build(
        doc.asOperation(), doc.getSchema());
    return createTrivialSequencer(doc,
        new OperationSink<DocOp>() {
