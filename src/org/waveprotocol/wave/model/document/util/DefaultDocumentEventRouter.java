 ...
 */
public class DefaultDocumentEventRouter<N, E extends N, T extends N>
    implements DocumentEventRouter<N, E, T>, DocumentHandler<N, E, T> {

  private static class Registration<T>
      implements ListenerRegistration {

    public static <T> Registration<T> create(
        DefaultDocumentEventRouter<?, ?, ?> router,
        Map<?, CopyOnWriteSet<T>> listenerMap,
        Object key,
        T listener) {
  ...
  @Override
  public void onDocumentEvents(EventBundle<N, E, T> bundle, WaveletOperationContext opContext) {
        ...
        if (event.getType() == Type.CONTENT_DELETED) {
          ...
          if (listeners != null) {
            for (ElementListener<E> listener : listeners) {
              listener.onElementRemoved(deleted, opContext);
            }
          }
        }
      }
    ...
    if (elementListenerMap != null) {
      for (DocumentEvent<N, E, T> event : bundle.getEventComponents()) {
        if (event.getType() == Type.CONTENT_INSERTED) {
          ...
          if (listeners != null) {
            for (ElementListener<E> listener : listeners) {
              listener.onElementAdded(inserted, opContext);
            }
          }
        }
      }
