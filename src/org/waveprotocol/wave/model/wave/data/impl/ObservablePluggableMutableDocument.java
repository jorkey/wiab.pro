 ...
 */
public class ObservablePluggableMutableDocument extends PluggableMutableDocument
    implements ObservableDocumentOperationSink<Doc.N, Doc.E, Doc.T>, ObservableDocument {
  ...
  private final static class DocumentHandlerManager // \u2620
    ...
    @Override
    public void onDocumentEvents(EventBundle event, WaveletOperationContext opContext) {
      if (isPaused) {
        return;
      }
      for (DocumentHandler<Doc.N, Doc.E, Doc.T> handler : handlers) {
        handler.onDocumentEvents(event, opContext);
      }
    }
