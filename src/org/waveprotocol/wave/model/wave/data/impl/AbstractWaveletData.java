 ...
 * via the protected
 * {@link #internalAddDocument(String, ParticipantId, Collection, DocumentOperationSink, long, long)
 * )} method.
 *
...
public abstract class AbstractWaveletData<B extends BlipData> implements ObservableWaveletData {
  ...
  final DocumentFactory<?> contentFactory;
   ...
   */
  protected abstract B internalCreateBlip(String docId, ParticipantId author,
  ...
  @Override
  public abstract B getBlip(String documentId);

  @Override
  public ImmutableSet<ParticipantId> getParticipants() {
    return ImmutableSet.copyOf(getMutableParticipants());
  }

  @Override
  public boolean addParticipant(ParticipantId p, WaveletOperationContext opContext) {
    Set<ParticipantId> participants = getMutableParticipants();
    if (participants.contains(p)) {
      return false;
    }
    participants.add(p);
    getListenerManager().onParticipantAdded(p, opContext);
    return true;
  }

  @Override
  public boolean addParticipant(
      ParticipantId p, int position, WaveletOperationContext opContext) {
    Set<ParticipantId> participants = getMutableParticipants();
    if (participants.contains(p)) {
      return false;
    }
  ...
  final public long setLastModifiedTime(long newTime) {
    ...
    listenerManager.onLastModifiedTimeChanged(oldTime, newTime);
    return oldTime;
  }

  @Override
  public B createBlip(String docId, ParticipantId author,
      Collection<ParticipantId> contributors, DocInitialization content,
      long creationTime, long creationVersion,
      long lastModifiedTime, long lastModifiedVersion) {
    B doc = internalCreateBlip(docId, author, contributors, content,
      creationTime, creationVersion, lastModifiedTime, lastModifiedVersion);
    getListenerManager().onBlipDataAdded(doc);
    return doc;
  }
   ...
   * BlipDataImpl.
   */
  public WaveletDataListenerManager getListenerManager() {
  ...
  protected void copyDocuments(ReadableWaveletData source) {
    for (String docId : source.getDocumentIds()) {
      ReadableBlipData docData = source.getBlip(docId);
      this.createBlip(docData.getId(), docData.getAuthor(), docData.getContributors(),
          docData.getContent().asOperation(),
          docData.getCreationTime(), docData.getCreationVersion(),
          docData.getLastModifiedTime(), docData.getLastModifiedVersion());
    }
  }
