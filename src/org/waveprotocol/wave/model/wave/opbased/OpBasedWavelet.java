...
public class OpBasedWavelet implements ObservableWavelet {
  ...
  private final WaveletDataListener eventForwarder = new WaveletDataListener() {
    @Override
    public void onParticipantAdded(ParticipantId participantId, WaveletOperationContext opContext) {
      for (WaveletListener l : listeners) {
        l.onParticipantAdded(OpBasedWavelet.this, participantId, opContext);
      }
    }

    @Override
    public void onParticipantRemoved(ParticipantId participantId, WaveletOperationContext opContext) {
      for (WaveletListener l : listeners) {
        l.onParticipantRemoved(OpBasedWavelet.this, participantId, opContext);
      }
    }
    ...
    public void onTagAdded(String tag, WaveletOperationContext opContext) {
      for (WaveletListener l : listeners) {
        l.onTagAdded(OpBasedWavelet.this, tag, opContext);
      }
    }
    ...
    public void onTagRemoved(String tag, WaveletOperationContext opContext) {
      for (WaveletListener l : listeners) {
        l.onTagRemoved(OpBasedWavelet.this, tag, opContext);
      }
    }
    ...
    @Override
    public void onBlipDataVersionModified(BlipData b, long oldVersion, long newVersion) {
      OpBasedBlip oblip = adapt(b);
      for (WaveletListener l : listeners) {
    ...
    @Override
    public void onBlipDataContributorAdded(BlipData blip, ParticipantId contributorId) {
      OpBasedBlip oblip = adapt(blip);
      for (WaveletListener l : listeners) {
    ...
    public void onRemoteBlipDataContentModified(BlipData blip) {
      OpBasedBlip oblip = adapt(blip);
      for (WaveletListener l : listeners) {
        l.onRemoteBlipContentModified(oblip);
      }
    }
  };
  ...
  public Iterable<? extends Blip> getBlips() {
    ...
    for (String documentId : wavelet.getDocumentIds()) {
      blips.add(adapt(wavelet.getBlip(documentId)));
    }
    return blips;
  }
  ...
  public OpBasedBlip getBlip(String blipId) {
    BlipData blipData = wavelet.getBlip(blipId);
    if (blipData != null) {
  ...
  public OpBasedBlip createBlip(String id) {
    ...
    BlipData newBlip = wavelet.createBlip(id, context.getCreator(),
        context.getCreator() != null ? Collections.singleton(context.getCreator()) : Collections.EMPTY_LIST,
        EmptyDocument.EMPTY_DOCUMENT, Constants.NO_TIMESTAMP, Constants.NO_VERSION, Constants.NO_TIMESTAMP, Constants.NO_VERSION);
  ...
  public void addParticipant(ParticipantId participant) {
    if (!wavelet.getParticipants().contains(participant)) {
      // Authorise and apply/send the op in separate stages to avoid sending
  ...
  public Set<String> getTags() {
    return Collections.unmodifiableSet(wavelet.getTags());
  }
