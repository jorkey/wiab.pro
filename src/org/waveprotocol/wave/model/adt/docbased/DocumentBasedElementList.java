...
public final class DocumentBasedElementList<E, T, I> implements ObservableElementList<T, I>,
  ...
  public void onElementAdded(E element, WaveletOperationContext opContext) {
    ...
    if (childTag.equals(getDocument().getTagName(element))) {
      ...
      int index = sibling == null ? 0 : orderedValues.indexOf(sibling) + 1;
      orderedValues.add(index, child);
      elementToValue.put(element, child);
      valueToElement.put(child, element);
      fireOnValueAdded(child, sibling, opContext);
    }
  }
  ...
  public void onElementRemoved(E element, WaveletOperationContext opContext) {
    T child = elementToValue.get(element);
    fireOnBeforeValueRemoved(child, opContext);
    elementToValue.remove(element);
    valueToElement.remove(child);
    orderedValues.remove(child);
    fireOnValueRemoved(child, opContext);
  }
  ...
  private void fireOnValueAdded(T child, T previous, WaveletOperationContext opContext) {
    for (ObservableElementList.Listener<? super T> l : listeners) {
      l.onValueAdded(child, previous, opContext);
    }
  }

  private void fireOnBeforeValueRemoved(T child, WaveletOperationContext opContext) {
    for (ObservableElementList.Listener<? super T> l : listeners) {
      l.onBeforeValueRemoved(child, opContext);
    }
  }

  private void fireOnValueRemoved(T child, WaveletOperationContext opContext) {
    for (ObservableElementList.Listener<? super T> l : listeners) {
      l.onValueRemoved(child, opContext);
    }
  }
}
