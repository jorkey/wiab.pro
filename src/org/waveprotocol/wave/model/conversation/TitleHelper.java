...
public final class TitleHelper {
   ...
   * null.
   */
  public static final String TITLE_KEY = Annotations.join(Blips.ANNOTATION_PREFIX , "title");
  ...
  static {
    ...
    b.annotationBoundary(AnnotationBoundaryMapImpl.builder()
        .initializationEnd(TITLE_KEY).build());
  ...
  public static String extractTitle(WaveContext wave) {
    if (wave == null) {
      return EMPTY_STRING;
    }
    return extractTitle(wave.getConversations());
  }

  public static String extractTitle(ConversationView conversations) {
    ...
    if (!firstBlip.isContentInitialized()) {
      return UNKNOWN_STRING;
    }
    
    Document content = firstBlip.getDocument();
    if (content == null) {
      return EMPTY_STRING;
    }
  ...
  public static <N, E extends N, T extends N> String extractTitle(ReadableDocument<N, E, T> doc) {
    ...
    String title = DocHelper.getText(doc, start, Annotations.firstAnnotationBoundary(
        doc, start, doc.size(), TITLE_KEY, AUTO_VALUE));
    if (title == null) {
      title = EMPTY_STRING;
    }
    ...
    outer: for (N node : DocIterate.deep(doc, Blips.getBody(doc), null)) {
      ...
      if (el == null) {
        if (firstLineReached != null) {
          text = doc.getData(doc.asText(node));
...
//          }
        ...
        if (doc.getTagName(el).equals(LineContainers.LINE_TAGNAME)) {
          if (text.length() == 0) { //the found line is empty => continue search
...
//  }
  ...
  public static String extractSnippet(WaveContext wave) {
    WaveView waveView = wave.getWave();
    if (waveView == null) {
      return EMPTY_STRING;
    }
    
    Wavelet wavelet = waveView.getRoot();
    if (wavelet == null) {
      return EMPTY_STRING;
    }
    
    WaveletData waveletData = wavelet.getWaveletData();
    if (waveletData == null) {
      return EMPTY_STRING;
    }
    ...
    String title = extractTitle(wave);
    if (isUnknown(title)) {
      return UNKNOWN_STRING;
    }
    
    ConversationView conversations = wave.getConversations();
    if (conversations.getRoot() == null) {
      return UNKNOWN_STRING;
    }
    
    String snippet = Snippets.renderSnippet(wave.getConversations(),
        Snippets.DIGEST_SNIPPET_LENGTH, title);
    return snippet;
  }

  public static boolean isUnknown(String s) {
    return s == UNKNOWN_STRING;
  }

  public static boolean isEmpty(String s) {
    return !isUnknown(s) && s.equals(EMPTY_STRING);
  }
