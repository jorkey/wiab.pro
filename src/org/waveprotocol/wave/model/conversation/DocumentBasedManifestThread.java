...
final class DocumentBasedManifestThread implements ObservableManifestThread {
  ...
  DocumentBasedManifestThread(ObservableElementList<ObservableManifestBlip, String> blips,
    ...
    ObservableElementList.Listener<ObservableManifestBlip> blipListListener =
        new ObservableElementList.Listener<ObservableManifestBlip>() {
          ...
          @Override
          public void onValueAdded(ObservableManifestBlip entry, ObservableManifestBlip previous,
              WaveletOperationContext opContext) {
            triggerOnManifestBlipAdded(entry, previous, opContext);
          }

          @Override
          public void onBeforeValueRemoved(ObservableManifestBlip entry, WaveletOperationContext opContext) {
            triggerOnBeforeManifestBlipRemoved(entry, opContext);
          }

          @Override
          public void onValueRemoved(ObservableManifestBlip entry, WaveletOperationContext opContext) {
            entry.detachListeners();
            triggerOnManifestBlipRemoved(entry, opContext);
          }
        };
  ...
  private void triggerOnManifestBlipAdded(ObservableManifestBlip entry,
      ObservableManifestBlip previous, WaveletOperationContext opContext) {
    for (Listener l : listeners) {
      l.onBlipAdded(entry, previous, opContext);
    }
  }

  @VisibleForTesting
  void triggerOnBeforeManifestBlipRemoved(ObservableManifestBlip entry,
      WaveletOperationContext opContext) {
    for (Listener l : listeners) {
      l.onBeforeBlipRemoved(entry, opContext);
    }
  }

  @VisibleForTesting  
  void triggerOnManifestBlipRemoved(ObservableManifestBlip entry, WaveletOperationContext opContext) {
    for (Listener l : listeners) {
      l.onBlipRemoved(entry, opContext);
    }
  }
}
