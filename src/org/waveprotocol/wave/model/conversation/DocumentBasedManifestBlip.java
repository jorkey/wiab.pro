...
final class DocumentBasedManifestBlip implements ObservableManifestBlip {
  /**
   * Initialization data for a thread.
  ...
  DocumentBasedManifestBlip(
    ...
    ObservableElementList.Listener<ObservableManifestThread> repliesListener =
      ...
      @Override
      public void onValueAdded(ObservableManifestThread entry, ObservableManifestThread previous,
          WaveletOperationContext opContext) {
        triggerOnManifestThreadAdded(entry, previous, opContext);
      }

      @Override
      public void onBeforeValueRemoved(ObservableManifestThread entry,
          WaveletOperationContext opContext) {
        triggerOnBeforeManifestThreadRemoved(entry, opContext);
      }
      ...
      public void onValueRemoved(ObservableManifestThread entry,
        ...
        triggerOnManifestThreadRemoved(entry, opContext);
      }
    };
  ...
  private void triggerOnManifestThreadAdded(ObservableManifestThread entry,
      ObservableManifestThread previous, WaveletOperationContext opContext) {
    for (Listener l : listeners) {
      l.onReplyAdded(entry, previous, opContext);
    }
  }

  @VisibleForTesting
  void triggerOnBeforeManifestThreadRemoved(ObservableManifestThread entry,
      WaveletOperationContext opContext) {
    for (Listener l : listeners) {
      l.onBeforeReplyRemoved(entry, opContext);
    }
  }

  @VisibleForTesting
  void triggerOnManifestThreadRemoved(ObservableManifestThread entry,
      WaveletOperationContext opContext) {
    for (Listener l : listeners) {
      l.onReplyRemoved(entry, opContext);
    }
  }
}
