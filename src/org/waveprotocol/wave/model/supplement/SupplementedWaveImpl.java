...
public class SupplementedWaveImpl implements SupplementedWave {
  ...
  static class WaveViewAdapter implements SupplementWaveView {
    ...
    public Iterable<WaveletId> getWavelets() {
      List<WaveletId> ids = new ArrayList<>();
    ...
    public HashedVersion getSignature(WaveletId id) {
      ...
      return (null != c) ?
          toWaveletBasedConversation(c).getWavelet().getHashedVersion()
    ...
    public long getVersion(WaveletId id) {
      ...
      return c != null ?
          ...
          toWaveletBasedConversation(c).getWavelet().getHashedVersion().getVersion()
    ...
    private WaveletBasedConversation toWaveletBasedConversation(Conversation c) {
      return c instanceof WaveletBasedConversation ? (WaveletBasedConversation) c
          : ((QuasiConversationImpl) c).getBaseConversation();
    }
  }
  ...
  private static class CheckingSupplementWaveView implements SupplementWaveView {
    ...
    public CheckingSupplementWaveView(SupplementWaveView target) {
      this.target = target;
    }
  ...
  public boolean isUnread(ConversationBlip blip) {
    return supplement.isBlipUnread(blip.getWavelet().getId(), blip.getId(),
        blip.getLastModifiedVersion());
  }

  @Override
  public boolean wasBlipEverRead(ConversationBlip blip) {
    return supplement.wasBlipEverRead(blip.getWavelet().getId(), blip.getId(),
        blip.getCreationVersion());
  }

  @Override
  public boolean isBlipLooked(ConversationBlip blip) {
    return supplement.isBlipLooked(blip.getWavelet().getId(), blip.getId(), blip.getCreationVersion());
  }

  @Override
  public boolean isBlipLooked(WaveletId waveletId, String blipId, long version) {
    return supplement.isBlipLooked(waveletId, blipId, version);
  }

  @Override
  public boolean isWaveletLooked(WaveletId waveletId) {
    return supplement.isWaveletLooked(waveletId);
  }

  @Override
  public long getLastReadWaveletVersion(WaveletId waveletId) {
    return supplement.getLastReadWaveletVersion(waveletId);
  }

  @Override
  public String getFocusedBlipId(WaveletId waveletId) {
    return supplement.getFocusedBlipId(waveletId);
  }

  @Override
  public ScreenPosition getScreenPosition(WaveletId waveletId) {
    return supplement.getScreenPosition(waveletId);
  }

  @Override
  public boolean isParticipantsUnread(WaveletId waveletId) {
    return supplement.isParticipantsUnread(waveletId, wave.getVersion(waveletId));
  }

  @Override
  public boolean haveParticipantsEverBeenRead(WaveletId waveletId) {
    return supplement.haveParticipantsEverBeenRead(waveletId);
  }

  @Override
  public boolean isTagsUnread(WaveletId waveletId) {
    return supplement.isTagsUnread(waveletId, wave.getVersion(waveletId));
  }

  @Override
  public boolean wasTagsEverRead(WaveletId waveletId) {
    return supplement.wasTagsEverRead(waveletId);
  }
  ...
  public void markAsRead() {
    for (WaveletId waveletId : wave.getWavelets()) {
      supplement.markWaveletAsRead(waveletId, wave.getVersion(waveletId));
    }
  }
  ...
  public void markParticipantsAsRead(WaveletId waveletId) {
    supplement.markParticipantsAsRead(waveletId, wave.getVersion(waveletId));
  }

  @Override
  public void markTagsAsRead(WaveletId waveletId) {
    supplement.markTagsAsRead(waveletId, wave.getVersion(waveletId));
  }
  ...
  public void markAsRead(ConversationBlip b) {
    ...
    if (isUnread(b)) {
      supplement.markBlipAsRead(b.getWavelet().getId(), b.getId(),
          ...
          Math.max(b.getLastModifiedVersion(), b.getWavelet().getVersion()));
    }
  }
  ...
  public void setFocusedBlipId(WaveletId waveletId, String blipId) {
    supplement.setFocusedBlipId(waveletId, blipId);
  }
  ...
  public void see(WaveletId waveletId) {
    supplement.setSeenVersion(waveletId, wave.getSignature(waveletId));
    supplement.clearPendingNotification();
  }

  @Override
  public void firstLookWavelet(WaveletId waveletId) {
    supplement.setFirstLookWaveletVersion(waveletId, wave.getVersion(waveletId));
    supplement.clearPendingNotification();
  }

  @Override
  public void firstLookBlip(ConversationBlip blip) {
    supplement.setFirstLookBlipVersion(blip.getWavelet().getId(), blip.getId(), blip.getWavelet().getVersion());
    supplement.clearPendingNotification();
  }
  ...
  public void archive() {
    if (isFollowed()) {
      for (WaveletId id : wave.getWavelets()) {
        supplement.archive(id, wave.getVersion(id));
      }
    } else {
      // Ignore.
  ...
  public boolean isArchived() {
    ...
    for (WaveletId id : wave.getWavelets()) {
      if (!supplement.isArchived(id, wave.getVersion(id))) {
        return false;
      }
    }
    return true;
  ...
  public WantedEvaluationSet getWantedEvaluationSet(WaveletId waveletId) {
    return supplement.getWantedEvaluationSet(waveletId);
  }
  ...
  public void markAsNotified() {
    for (WaveletId id : wave.getWavelets()) {
      supplement.markWaveletAsNotified(id, wave.getVersion(id));
    }
  }
