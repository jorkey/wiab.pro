...
public final class WaveletBasedSupplement implements ObservablePrimitiveSupplement {
  ...
  public static final String LOOKSTATE_DOCUMENT = "m/look";
  ...
  public static final String BLIPS_DOCUMENT = "m/blips";
  ...
  public static final String BLIP_LOOK_TAG = "blip_look";  
  ...
  public static final String WAVELET_LOOK_TAG = "all_look";
  ...
  public static final String FOCUSED_ATTR = "focused";
  public static final String SCREEN_ATTR = "screen";

  /** Collection of per-wavelet read states. */
  private final WaveletReadStateCollection<?> read;

  /** Collection of per-wavelet look states. */
  private final WaveletLookStateCollection<?> look;
  ...
  private final ObservableMonotonicMap<WaveletId, Long> waveletArchiveVersions;
  ...
  private final ObservableBasicMap<WaveletId, Long> notifiedVersion;
  ...
  /** Blip states (focused and top screen blips). */
  private final WaveletBlipStateCollection<?> blipStates;
  ...
  private final Listener forwardingListener = new Listener() {
    ...
    public void onFirstLookBlipVersionSet(WaveletId wid, String bid, long newVersion) {
      triggerOnFirstLookBlipVersionSet(wid, bid, newVersion);
    }
    
    @Override
    public void onLastReadParticipantsVersionChanged(
        WaveletId wid, long oldVersion, long newVersion) {
    ...
    public void onFirstLookWaveletVersionSet(WaveletId wid, long newVersion) {
      triggerOnFirstLookWaveletVersionSet(wid, newVersion);
    }
    ...
    public void onFocusedBlipIdChanged(
      ...
      triggerOnFocusedBlipIdChanged(wid, oldFocusedBlipId, newFocusedBlipId);
    }

    @Override
    public void onScreenPositionChanged(
        WaveletId wid, ScreenPosition oldScreenPosition, ScreenPosition newScreenPosition) {
      triggerOnScreenPositionChanged(wid, oldScreenPosition, newScreenPosition);
    }
  };
  ...
  private void installListeners() {
    ...
    waveletArchiveVersions.addListener(new ObservableMonotonicMap.Listener<WaveletId, Long>() {
      
      @Override
      public void onEntrySet(WaveletId waveletId, Long oldValue, Long newValue) {
  ...
  public void setFirstLookBlipVersion(WaveletId waveletId, String blipId, long version) {
    look.setFirstLookBlipVersion(waveletId, blipId, version);
  }
  ...
  public void setFirstLookWaveletVersion(WaveletId waveletId, long version) {
    look.setFirstLookWaveletVersion(waveletId, version);
  }

  @Override
  public void clearWaveletLookState(WaveletId waveletId) {
    look.clearWaveletLookState(waveletId);
  }
  ...
  public void clearBlipLookState(WaveletId waveletId, String blipId) {
    look.clearBlipLookState(waveletId, blipId);
  }
  ...
  public long getFirstLookBlipVersion(WaveletId waveletId, String blipId) {
    return look.getFirstLookBlipVersion(waveletId, blipId);
  }
  ...
  public long getFirstLookWaveletVersion(WaveletId waveletId) {
    return look.getFirstLookWaveletVersion(waveletId);
  }
  ...
  public long getArchiveWaveletVersion(WaveletId waveletId) {
    ...
    Long version = waveletArchiveVersions.get(waveletId);
    return version != null ? version : NO_VERSION;
  }
  ...
  public long getNotifiedVersion(WaveletId waveletId) {
    Long version = notifiedVersion.get(waveletId);
    return version != null ? version : NO_VERSION;
  }

  @Override
  public void setNotifiedVersion(WaveletId waveletId, long signature) {
  ...
  private void triggerOnLastReadBlipVersionChanged(
      WaveletId waveletId, String blipId, long oldVersion, long newVersion) {
    for (Listener listener : listeners) {
      listener.onLastReadBlipVersionChanged(waveletId, blipId, oldVersion, newVersion);
    }
  }

  private void triggerOnFirstLookBlipVersionSet(
      WaveletId waveletId, String blipId, long newVersion) {
    for (Listener listener : listeners) {
      listener.onFirstLookBlipVersionSet(waveletId, blipId, newVersion);
    }
  }
  
  private void triggerOnLastReadParticipantsVersionChanged(
      WaveletId waveletId, long oldVersion, long newVersion) {
    for (Listener listener : listeners) {
  ...
  private void triggerOnFirstLookWaveletVersionSet(
      WaveletId waveletId, long newVersion) {
    for (Listener listener : listeners) {
      listener.onFirstLookWaveletVersionSet(waveletId, newVersion);
    }
  }  
  ...
  private void triggerOnFocusedBlipIdChanged(
      WaveletId waveletId, String oldFocusedBlipId, String newFocusedBlipId) {
    for (Listener listener : listeners) {
      listener.onFocusedBlipIdChanged(waveletId, oldFocusedBlipId, newFocusedBlipId);
    }
  }

  private void triggerOnScreenPositionChanged(
      WaveletId waveletId, ScreenPosition oldScreenPosition, ScreenPosition newScreenPosition) {
    for (Listener listener : listeners) {
      listener.onScreenPositionChanged(waveletId, oldScreenPosition, newScreenPosition);
    }
  }
   ...
   */
  private static <E> ObservableMonotonicMap<WaveletId, Long> createWaveletArchiveState(
      DocumentEventRouter<? super E, E, ?> router) {
    return DocumentBasedMonotonicMap.create(router, router.getDocument().getDocumentElement(),
        WaveletIdSerializer.INSTANCE, Serializer.LONG, ARCHIVE_TAG, ID_ATTR, VERSION_ATTR);
  }
  ...
  /**
   * Exposes a document as a collection of per-wavelet look-state objects.
   *
   * @param router router for look-state document
   * @return wavelet look-state.
   */
  private static <E> WaveletLookStateCollection<E> createWaveletLookState(
      DocumentEventRouter<? super E, E, ?> router, Listener listener) {
    E container = router.getDocument().getDocumentElement();
    return WaveletLookStateCollection.create(router, container, listener);
  }
   ...
   */
  private static <E> ObservableBasicMap<WaveletId, Long> createWaveletNotifiedVersion(
      DocumentEventRouter<? super E, E, ?> router) {
    return DocumentBasedMonotonicMap.create(router, router.getDocument().getDocumentElement(),
        WaveletIdSerializer.INSTANCE, Serializer.LONG, NOTIFIED_VERSION_TAG, ID_ATTR,
        VERSION_ATTR);
  }
  ...
  /**
   * Exposes a document as blip states.
   *
   * @param router router for the gadget state document.
   * @param listener event listener.
   * @return blip state collection object to access blip states.
   */
  private static <E> WaveletBlipStateCollection<E> createBlipStatesDoc(
      DocumentEventRouter<? super E, E, ?> router, Listener listener) {
    return WaveletBlipStateCollection.create(
        router, router.getDocument().getDocumentElement(), listener);
  }
  ...
  private static <N> ObservableMonotonicMap<WaveletId, Long> fungeCreateWaveletArchiveState(
      ObservableMutableDocument<N, ?, ?> doc) {
  ...
  private static <N> WaveletLookStateCollection<?> fungeCreateLookState(
      ObservableMutableDocument<N, ?, ?> doc, Listener listener) {
    return createWaveletLookState(DefaultDocumentEventRouter.create(doc), listener);
  }
  ...
  private static <N> ObservableBasicMap<WaveletId, Long> fungeCreateNotifiedVersion(
      ObservableMutableDocument<N, ?, ?> doc) {
  ...
  private static <N> WaveletBlipStateCollection<?> fungeCreateBlipStates(
      ObservableMutableDocument<N, ?, ?> doc, Listener listener) {
    return createBlipStatesDoc(DefaultDocumentEventRouter.create(doc), listener);
  }
}
