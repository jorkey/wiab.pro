...
/**
 * Implements the per-wavelet look state.
 *
 * @param <E> element type of the document implementation
 * 
 * @author dyukon@gmail.com (Denis Konovalchik)
 */
class DocumentBasedWaveletLookState<E> implements WaveletLookState {
  ...
  DocumentBasedWaveletLookState(DocumentEventRouter<? super E, E, ?> router, E container) {
    this.router = router;
   ...
   */
  public static <E> DocumentBasedWaveletLookState<E> create(
      DocumentEventRouter<? super E, E, ?> router, E container, WaveletId id,
      Listener listener) {
    DocumentBasedWaveletLookState<E> x = new DocumentBasedWaveletLookState<>(router, container);
  ...
  private void installListeners(final WaveletId wid, final Listener listener) {
    ...
    blips.addListener(new ObservableMonotonicMap.Listener<String, Long>() {
      ...
      public void onEntrySet(String key, Long oldValue, Long newValue) {
        listener.onFirstLookBlipVersionSet(wid, key, valueOf(newValue));
      }
    });
    ...
    wavelet.addListener(new ObservableMonotonicValue.Listener<Long>() {
      ...
      public void onSet(Long oldValue, Long newValue) {
        listener.onFirstLookWaveletVersionSet(wid, valueOf(newValue));
      }
    });
  }
  ...
  @Override
  public void setBlipFirstLookVersion(String blipId, long version) {
    blips.put(blipId, version);
  }
  
  @Override
  public long getBlipFirstLookVersion(String id) {
    return valueOf(blips.get(id));
  }
  
  @Override
  public void setWaveletFirstLookVersion(long version) {
    wavelet.set(version);
  }  

  @Override
  public long getWaveletFirstLookVersion() {
    return valueOf(wavelet.get());
  }
  
  @Override
  public Iterable<String> getLookedBlips() {
    return blips.keySet();
  }
  ...
  @Override
  public void clearWaveletLookState() {
    wavelet.set(PrimitiveSupplement.NO_VERSION);
  }

  @Override
  public void clearBlipLookState(String blipId) {
    blips.remove(blipId);
  }
}
