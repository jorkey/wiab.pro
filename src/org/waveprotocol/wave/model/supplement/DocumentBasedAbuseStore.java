...
public final class DocumentBasedAbuseStore<N, E extends N> implements ObservableAbuseStore {
  ...
  private DocumentBasedAbuseStore(
    ...
    this.list.addListener(new ObservableElementList.Listener<WantedEvaluation>() {
      @Override
      public void onValueAdded(WantedEvaluation entry, WantedEvaluation previous,
          WaveletOperationContext opContext) {
        triggerEvaluationAdded(entry);
      }

      @Override
      public void onBeforeValueRemoved(WantedEvaluation entry, WaveletOperationContext opContext) {
      }

      @Override
      public void onValueRemoved(WantedEvaluation entry, WaveletOperationContext opContext) {
