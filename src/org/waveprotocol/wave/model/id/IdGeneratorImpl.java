...
public class IdGeneratorImpl implements IdGenerator, IdConstants {
  ...
  public WaveletId newUserDataWaveletId(String address) {
    ...
    // Commented by A. Kaplanov to work with external authorization.
    // String userDomain = ParticipantId.ofUnsafe(address).getDomain();
    
    return WaveletId.of(defaultDomain, build(USER_DATA_WAVELET_PREFIX, address));
  }
  ...
  public String newBlockId() {
    String newId = newId(BLOCK_PREFIX);
    if (!IdUtil.isBlockId(newId)) {
      Preconditions.illegalState("generated blip id '" + newId + "' is not a block id");
    }
    return newId;
  }
