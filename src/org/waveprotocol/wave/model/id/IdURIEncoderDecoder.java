...
/**
 * This class is used to generate URI for id's.
 *
...
public class IdURIEncoderDecoder {
  ...
  /**
   * @param name The WaveletName to serialize. The domain name in WaveletId and WaveId
   ...
   */
  public String waveletNameToURI(WaveletName name)
   ...
   * @throws EncodingException This can happen if the values in name are not valid UTF-16.
   */
  public String waveletNameToURIPath(WaveletName name)
      throws EncodingException {
    ...
    return waveletId.getDomain() + "/" + waveDomain + 
        encoder.encode(waveId.getId()) +
        "/" + encoder.encode(waveletId.getId());
  }


  /**
   * @param serializedForm The serialized URI form of the wavelet name.
   * @return The WaveletName that is deserialized.
   *
   ...
   *    by a hex value.
   */
  public WaveletName uriToWaveletName(String serializedForm)
      throws EncodingException {
    String prefix = IdConstants.WAVE_URI_SCHEME + "://";
    if (!serializedForm.startsWith(prefix)) {
      throw new IllegalArgumentException(
          "Invalid scheme for the wavelet name URI: " + serializedForm);
    }
    return uriPathToWaveletName(serializedForm.substring(prefix.length()));
  }

  /**
   * @param serializedForm The serialized URI path part only of the wavelet name.
   * @return The WaveletName that is deserialized.
   * @throws EncodingException
   *
   * @throws EncodingException if decoding fails. e.g. the hex values following percent in
   ...
   *    by a hex value.
   */
  public WaveletName uriPathToWaveletName(String serializedForm)
      throws EncodingException {
    String[] parts = serializedForm.split("/");
    if (parts.length != 3) {
      throw new IllegalArgumentException(
          "Wavelet name URI path expected in the format of" +
          " <wavelet domain>/[<wave domain>!]<wave id>/<wavelet id> but got: " +
          serializedForm);
    }

    WaveletId waveletId =
        WaveletId.ofLegacy(parts[0], encoder.decode(parts[2]));

    String[] waveIdParts =
        SimplePrefixEscaper.DEFAULT_ESCAPER.splitWithoutUnescaping(
        '!', parts[1]);
    WaveId waveId;
    if (waveIdParts.length == 1) {
      waveId = WaveId.ofLegacy(
          waveletId.getDomain(), encoder.decode(waveIdParts[0]));
    } else if (waveIdParts.length == 2) {
      waveId = WaveId.ofLegacy(
          waveIdParts[0], encoder.decode(waveIdParts[1]));
    } else {
      throw new IllegalArgumentException(
          "Wave id in URI path is invalid. Expected the format" +
