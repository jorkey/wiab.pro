...
/**
 * Makes passive wavelet objects ({@link ObservableWaveletFragmentData}) operational,
...
public final class WaveletOperationalizer {
   ...
   */
  public static WaveletOperationalizer create(WaveId wave, Factory opContextFactory) {
    return new WaveletOperationalizer(wave, opContextFactory);
  }  
  
  private final WaveId waveId;
  private final StringMap<LiveTarget<ObservableWaveletFragmentData, WaveletOperation>>
      wavelets = CollectionUtils.createStringMap();
   ...
   */
  public OpBasedWavelet operationalize(ObservableWaveletFragmentData data) {
    LiveTarget<ObservableWaveletFragmentData, WaveletOperation> target = createSinks(data);
  ...
  public Collection<ObservableWaveletFragmentData> getWavelets() {
    final Collection<ObservableWaveletFragmentData> targets = CollectionUtils.createQueue();
    this.wavelets.each(new ProcV<LiveTarget<ObservableWaveletFragmentData, WaveletOperation>>() {
      @Override
      public void apply(String id, LiveTarget<ObservableWaveletFragmentData, WaveletOperation> triple) {
  ...
  public Pair<SilentOperationSink<WaveletOperation>, ProxyOperationSink<WaveletOperation>> getSinks(
      String waveletId) {
    LiveTarget<ObservableWaveletFragmentData, WaveletOperation> target = wavelets.get(waveletId);
   ...
   */
  private LiveTarget<ObservableWaveletFragmentData, WaveletOperation> createSinks(
      ObservableWaveletFragmentData data) {
    return putAndReturn(wavelets,
        ModernIdSerialiser.INSTANCE.serialiseWaveletId(data.getWaveletId()),
        LiveTarget.<ObservableWaveletFragmentData, WaveletOperation>create(data));
  }
