...
public final class KeySignalRouter implements KeySignalHandler {
  ...
  /**
   * Registers a key handler for a set of tasks.
   *
   * @param context context of key combos
   * @param tasks tasks to handle
   * @param handler handler for {@code keys}
   */
  public void registerTasks(KeyComboContext context, Iterable<KeyComboTask> tasks,
      KeySignalHandler handler) {
    for (KeyComboTask task : tasks) {
      register(KeyComboManager.getKeyCombosByTask(context, task), handler);
    }
  }
