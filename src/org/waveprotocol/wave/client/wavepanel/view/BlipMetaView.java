...
public interface BlipMetaView extends View, IntrinsicBlipMetaView {
   ...
   * @param frame
   */
  void setFocusChrome(FocusFrameView frame, boolean on);

  /**
   * Determines if it contains the chrome that places focus on a blip.
   */  
  boolean hasFocusChrome();  
  
  /**
   * Determines if it contains the chrome that places focus on an blip being edited.
   */  
  boolean hasEditedFocusChrome();
    
  DraftModeControls attachDraftModeCotrols();
  void detachDraftModeControls();
  
  void showDraftModeControls();
  void hideDraftModeControls();
  boolean areDraftModeControlsVisible();
}
