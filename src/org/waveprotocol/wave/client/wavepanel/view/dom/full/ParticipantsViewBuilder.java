...
public final class ParticipantsViewBuilder implements UiBuilder {
  ...
  @VisibleForTesting
  private ParticipantsViewBuilder(Css css, String id, HtmlClosureCollection participantUis) {
    this.css = css;
    this.id = id;
  ...
  public void outputHtml(SafeHtmlBuilder output) {
    String addParticipantHotKey = KeyComboManager.getKeyComboHintByTask(
      ...
      open(output, null, css.flow(), null);
      {
        open(output, Components.CONTAINER.getDomId(id), null, null);
        {
          ...
          openSpan(output, null, css.extra(), null);
          {
            ...
            closeSpan(output);
            appendSpanWith(
                output, null, css.addButton(), TypeCodes.kind(Type.ADD_PARTICIPANT), null,
                messages.addParticipantHint(), addParticipantHotKey);
          }
          closeSpan(output);
          ...
          {
            appendSpanWith(
                output, WebClient.ADD_PARTICIPANT_BUTTON_ID, css.addButton(),
                TypeCodes.kind(Type.ADD_PARTICIPANT), null, messages.addParticipantHint(),
                addParticipantHotKey);
          }
          closeSpan(output);
        }
        close(output);
  ...
  private String onClickJs() {
    ...
    sb.append("var p=document.getElementById('").append(id).append("');")
        ...
        .append("p.nextSibling.nextSibling.style.top=")
        .append("p.clientHeight+p.nextSibling.clientHeight;");
