...
public final class DomViewHelper {
  ...
  public static Element load(String baseId, Component c) {
    String id = c.getDomId(baseId);
    Element e = Document.get().getElementById(id);
    if (e == null) {
      throw new RuntimeException("Component not found: " + c + ", id: " + id);
    }
    return e;
  }
  ...
  public static void insert(Element container, Element target, Element neighbor,
      boolean beforeNeighbor) {
    Preconditions.checkArgument(neighbor == null || neighbor.getParentElement().equals(container));
    Preconditions.checkArgument(target.getParentElement() == null);
    if (beforeNeighbor) {
      if (neighbor == null) {
        container.appendChild(target);
      } else {
        container.insertBefore(target, neighbor);
      }
    } else {
      if (neighbor == null) {
        container.insertFirst(target);
      } else {
        container.insertAfter(target, neighbor);
      }
    }
  }
