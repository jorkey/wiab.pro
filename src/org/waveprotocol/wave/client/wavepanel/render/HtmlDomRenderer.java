...
public final class HtmlDomRenderer implements DomRenderer {
  ...
  private HtmlDomRenderer(WaveRenderer<UiBuilder> driver) {
    this.driver = driver;
  }

  public static HtmlDomRenderer create(WaveRenderer<UiBuilder> driver) {
    return new HtmlDomRenderer(driver);
  }
  ...
  @Override
  public Element renderPlaceholder() {
    return parseHtml(driver.renderPlaceholder());
  }
  ...
  public Element render(Conversation conversation, ParticipantId participant,
    ...
    return parseHtml(driver.render(conversation, participant, state, hint));
  }

  @Override
  public Element renderParticipants(Conversation conversation) {
    return parseHtml(driver.renderParticipants(conversation));
  }

  @Override
  public Element renderRootThread(ConversationThread rootThread) {
    return parseHtml(driver.renderRootThread(rootThread));
  }

  @Override
  public Element renderTags(Conversation conversation) {
    return parseHtml(driver.renderTags(conversation));
  }

  @Override
  public Element render(Conversation conversation, String tag, TagState state, String hint) {
    return parseHtml(driver.render(conversation, tag, state, hint));
  }
