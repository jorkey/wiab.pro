...
public class ReplyManager implements InlineAnchorLiveRenderer.AnchorHandler {
  ...
  public InlineThreadView present(BlipView blipView, ConversationThread thread,
    ...
    AnchorView defaultUi = blipView.insertDefaultAnchor(thread, null, beforeNeighbor);
    InlineThreadView threadUi = defaultUi.getThread();    
    
    AnchorView inlineAnchorView = views.getInlineAnchor(thread);
    if (inlineAnchorView != null) {
      defaultUi.detach(threadUi);
      inlineAnchorView.attach(threadUi);
    }
    return threadUi;
  }  
}
