...
public final class DocumentUtil {
  ...
  /**
   * @return the nearest parent content element with paragraph tag
   * 
   * @param node start node
   */  
  public static ContentElement getParentParagraph(ContentNode node) {
    return getNearestParentWithTag(node, Paragraph.FULL_TAGNAME);
  }
  
  /**
   * @return the nearest parent content element with given tag
   * 
   * @param node start node
   * @param parentTag tag of the parent
   * 
   */
  public static ContentElement getNearestParentWithTag(ContentNode node, String parentTag) {
    for (ContentElement e = node.getParentElement(); e != null; e = e.getParentElement()) {
      if (parentTag.equals(e.getTagName()) ) {
        return e;
      }  
    }
    return null;
