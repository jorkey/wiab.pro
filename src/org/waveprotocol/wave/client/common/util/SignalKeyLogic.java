...
public final class SignalKeyLogic {
  ...
  public void computeKeySignalType(
    ...
    int typeInt;
    switch (typeName) {
      case JsEvents.KEY_DOWN:
        typeInt = Event.ONKEYDOWN;
        break;
      case JsEvents.KEY_PRESS:
        typeInt = Event.ONKEYPRESS;
        break;
      case JsEvents.KEY_UP:
        result.type = null;
        return;
      default:
    ...
    switch (userAgent) {
      case WEBKIT:
        ...
        if (isIME) {
          ...
          // Walkaround for Chrome 32+ for Windows where "keypress" event for "." symbol
          // contains keyIdentifier=DELETE_KEY_IDENTIFIER.
          ...
          // Walkaround for Chrome 32+ for Windows where "keypress" events for some input symbols ("(", "\"")
          // contain keyIdentifiers within NAVIGATION_KEY_IDENTIFIERS.
          if (typeInt != Event.ONKEYPRESS) {
  ...
  private static boolean isInputKeyCodeIE(int keyCode) {
    /*
    DATA
    ----
    For KEYDOWN:
