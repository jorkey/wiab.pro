...
public class SignalEventImpl implements SignalEvent {
  ...
  static {
    ...
    for (String e : new String[] {
        JsEvents.COMPOSITION_START,
        JsEvents.COMPOSITION_END,
        JsEvents.COMPOSITION_UPDATE,
        JsEvents.TEXT }) {
      COMPOSITION_EVENTS.add(e);
      CANCEL_BUBBLE_QUIRKS.add(e);
    }
    COMPOSITION_EVENTS.add(JsEvents.TEXT_INPUT);
    CANCEL_BUBBLE_QUIRKS.add(JsEvents.CONTEXT_MENU);
    for (String e : new String[] {
  ...
  protected static <T extends SignalEventImpl> T createInner(T ret,
    ...
    } else if ((UserAgent.isIE() ? JsEvents.PASTE : JsEvents.BEFORE_PASTE)
  ...
  public final boolean isClickEvent() {
    return JsEvents.CLICK.equals(getType());
  }
  ...
  public final boolean isPasteEvent() {
    return (UserAgent.isIE() ? JsEvents.BEFORE_PASTE : JsEvents.PASTE).equals(nativeEvent.getType());
  }
  ...
  public final boolean isCutEvent() {
    return (UserAgent.isIE() ? JsEvents.BEFORE_CUT : JsEvents.CUT).equals(nativeEvent.getType());
  }
  ...
  public final boolean isCopyEvent() {
    return JsEvents.COPY.equals(nativeEvent.getType());
  }
   ...
   * @param letter
   */
  private int comboInputKeyCode(char letter) {
  ...
  private boolean isPreventDefaultEffective() {
    if (QuirksConstants.PREVENT_DEFAULT_STOPS_CONTEXTMENT) {
      ...
      return !type.equals(JsEvents.CONTEXT_MENU);
    }
  }
