...
public interface WaveRenderer<T> {
  ...
  /** Renders specified participant. */
  T render(Conversation conversation, ParticipantId participant, ParticipantState state, String hint);

  /** Renders a tag. */
  T render(Conversation conversation, String tag, TagState state, String hint);

  /** Renders all participants of the conversation. */
  T renderParticipants(Conversation conversation);

  /** Renders a placeholder. */
  T renderPlaceholder();

  /** Renders root thread.
   * @return the rendering create {@code thread}, without a surrounding anchor.
   */
  T renderRootThread(ConversationThread rootThread);

  /** Renders all tags of the conversation. */
  T renderTags(Conversation conversation);
}
