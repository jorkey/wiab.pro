...
public final class ReducingRendererHelper<R> implements ResultProducingRenderHelper<R> {
  ...
  public void endConversation(Conversation conv) {
    ...
    for (ParticipantId participant : conv.getParticipantIds()) {
      ...
      allParticipants.put(name, builders.render(
          conv, participant, ParticipantState.NORMAL, name));
    }
    ...
    for (String tag : conv.getTags()) {
      allTags.put(tag, builders.renderTag(conv, tag, TagState.NORMAL, null));
    }
    R tags = builders.renderTags(conv, allTags);

    R rootThread = leave().getThreads().get(conv.getRootThread());
    currentScope().add(conv, builders.render(conv, participants, rootThread, tags));
  }
  ...
  public void endBlip(ConversationBlip blip) {
    ...
    currentScope().add(blip, builders.render(blip, document, defaultAnchors));
  }
  ...
  public void endThread(ConversationThread thread) {
    ...
    currentScope().add(thread, builders.render(thread, blips, true));
  }
