...
public abstract class LogLevel {
  ...
  public static boolean showErrors() {
    return getInstance().showErrorsInstance();
  }
  ...
  public static boolean showDebug() {
    return getInstance().showDebugInstance();
  }

  private static LogLevel getInstance() {
    if (instance == null) {
      if (GWT.isClient()) {
        switch (Flags.get().enableClientDebug()) {
          case ON:
            // Debug is switched on
            instance = new DebugImpl();
            break;
          case OFF:
            // Debug is switched off
            instance = new NoneImpl();
            break;
          default:
            // Dependingly on compilation settings
            instance = GWT.<LogLevel>create(LogLevel.class);
            break;
        }
      } else {
