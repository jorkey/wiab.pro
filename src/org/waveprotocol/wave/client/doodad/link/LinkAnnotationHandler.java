...
public class LinkAnnotationHandler implements AnnotationMutationHandler {
  ...
  private static void registerBehaviour(AnnotationRegistry registry, String prefix) {
    registry.registerBehaviour(prefix,
      ...
      @Override
      public AnnotationBehaviour.BiasDirection getBias(final StringMap<Object> left, final StringMap<Object> right,
          AnnotationBehaviour.CursorDirection cursor) {
        final Box<AnnotationBehaviour.BiasDirection> ret = Box.create(AnnotationBehaviour.BiasDirection.NEITHER);
        KEYS.each(new Proc() {
          @Override
          public void apply(String key) {
            if (left.get(key) != null) {
              ret.boxed = AnnotationBehaviour.BiasDirection.RIGHT;
            } else if (right.get(key) != null) {
              ret.boxed = AnnotationBehaviour.BiasDirection.LEFT;
            }
          }
        });
        return ret.boxed;
  ...
  private static class RenderFunc implements PaintFunction{
    private final LinkAnnotationHandler.LinkAttributeAugmenter augmenter;

    public RenderFunc(LinkAnnotationHandler.LinkAttributeAugmenter augmenter) {
