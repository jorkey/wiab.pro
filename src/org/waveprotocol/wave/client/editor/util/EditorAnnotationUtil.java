...
public class EditorAnnotationUtil {
   ...
   */
  public static void setAnnotationOverSelection(
      EditorContext editor, String key, String value, boolean wholeParagraph) {
    
    Range range = Preconditions.checkNotNull(
        editor.getSelectionHelper().getSelectionRange(), "Editor must have selection").asRange();

    if(wholeParagraph) {
      range = Paragraph.extendRange(editor.getDocument(), range);
    }
  ...
  /**
   * Clears all text style annotations for a set of keys over the current selected range.
   *
...
public static boolean clearTextStyleAnnotationsOverSelection(EditorContext editor) {  
  return clearAnnotationsOverSelection(editor, asArray(
