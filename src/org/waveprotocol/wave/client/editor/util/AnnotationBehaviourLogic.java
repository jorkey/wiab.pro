...
public class AnnotationBehaviourLogic<N> {
   ...
   */
  public void supplementAnnotations(
      final int location, final BiasDirection bias, final ContentType type) {
    Preconditions.checkState(doc != null, "Cannot call out of init/reset cycle.");
  ...
  private void buildBoundaryMaps(final int location) {
    ...
    if (location > 0) {
      doc.forEachAnnotationAt(location - 1,
          new ProcV<String>() {
    ...
    if (location < doc.size()) {
      doc.forEachAnnotationAt(location,
          new ProcV<String>() {
  ...
  /** Utility that takes a bias and inheritance, then returns whether to inherit from the right. */
