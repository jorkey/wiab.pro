...
import org.waveprotocol.wave.client.editor.i18n.EditorMessages;
...
public class EditorImpl extends LogicalPanel.Impl implements
  ...
  private static final EditorMessages messages = GWT.create(EditorMessages.class);  
  ...
  /** Buffer of outgoing ops, for when we defer sending them. */
  List<DocOp> deferredOutgoingOps = new LinkedList<>();

  /** Buffer of outgoing ops, for when we suppress sending them. */
  List<DocOp> suppressedOutgoingOps;

  // Editor format key combinations
  ...
  public void leaveDraftMode(boolean saveChanges) {
    ...
    if (!content.getDraftOps().isEmpty()) {
      if (saveChanges) {
        for (DocOp o : content.getDraftOps()) {
          innerOutputSink.consume(o);
        }
  ...
  public boolean isDraftModified() {
    return !content.getDraftOps().isEmpty();
  }

  void flushSink() {
    if (!deferredOutgoingOps.isEmpty()) {
      DocOp op = Composer.compose(deferredOutgoingOps);
      deferredOutgoingOps.clear();
      if (isDraftMode()) {
  ...
  private class EditorEventsSubHandlerImpl implements EditorEventsSubHandler {
    ...
    public boolean handleRangeKeyCombo(EditorEvent event, ContentRange selection) {
      ...
      switch (combo.getAssignedTask(KeyComboContext.TEXT_EDITOR)) {
        case FORMAT_BOLD:
     ...
     */
    private void doCollapsedStyle(Point<ContentNode> caret, String key, String value) {
      key = StyleAnnotationHandler.key(key);
  ...
  private final Task instrumentationTask = new Task() {
    ...
    @Override
    public void execute() {
      settings.getInstrumentor().recordDuration(TimedAction.INPUT_PROCESS, processKeyPressTimer);
  ...
  private final RepairListener repairListener = new RepairListener() {
    ...
    public void onFullDocumentRevert(
        ReadableDomDocument<ContentNode, ContentElement, ContentTextNode> doc) {
...
//        }
    ...
    public Point<ContentNode> deleteRange(Point<ContentNode> first, Point<ContentNode> second,
      ...
      final Point<ContentNode> filteredFirst = getFilteredPoint(first);
      final Point<ContentNode> filteredSecond = getFilteredPoint(second);      
      final Iterable<? extends ConversationThread> repliesToDelete = findInnerRepliesWithinRange(
          blip, mapper().getLocation(filteredFirst), mapper().getLocation(filteredSecond));

      if (repliesToDelete.iterator().hasNext()) {
        DialogBox.confirm(messages.confirmThreadDeletion(), new Command() {
          
          @Override
          public void execute() {
            deleteRangeInner(filteredFirst, filteredSecond, repliesToDelete);
          }
        });
        return filteredFirst;
      }
      
      return deleteRangeInner(filteredFirst, filteredSecond, repliesToDelete);
    }
    ...
    /**
     * Finds inner replies of the blip within given range.
     * 
     * @param blip
     * @param beginLocation begin location
     * @param endLocation end location
     * @return inner replies collection
     */
    private Iterable<? extends ConversationThread> findInnerRepliesWithinRange(
        ConversationBlip blip, int beginLocation, int endLocation) {
      List<ConversationThread> replies = new ArrayList<>();
      Iterator<? extends ConversationBlip.LocatedReplyThread> it =
          blip.locateReplyThreads().iterator();    
      while (it.hasNext()) {
        ConversationBlip.LocatedReplyThread locatedThread = it.next();
        int location = locatedThread.getLocation();
        if (location >= beginLocation && location < endLocation) {
          replies.add(locatedThread.getThread());
        }
      }
      return replies;
    }    
    
    private Point<ContentNode> deleteRangeInner(Point<ContentNode> first, Point<ContentNode> second,
        Iterable<? extends ConversationThread> repliesToDelete) {
      Point<ContentNode> result = mutable().deleteRange(first, second).getFirst();
      for (ConversationThread reply : repliesToDelete) {
        reply.delete();
      }
      return result;
    }
  ...
  /**
   * Conversation blip owning the document.
   */
   ...
   * Constructor.
   */
  protected EditorImpl(boolean ownsDocument, Element div, ConversationBlip blip) {
  ...
  public void setContent(ContentDocument newDoc) {
    ...
    try {
      ...
      typing = new TypingExtractor(typingSink, nodeManager,
          content.getFilteredHtmlView(), content.getRenderedView(), repairer, new SelectionSource() {
      ...
      if (settings.undoEnabled()) {
        editorUndoManager = new EditorUndoManagerImpl(
            UndoManagerFactory.createUndoManager(), new SilentOperationSink<DocOp>() {
...
//      /////////////////////////////
  ...
  public void setEditing(final boolean editing) {
    ...
    if (content != null) {
      ...
      if (editing) {
        ...
        // immediate save selection and cancel save selection task when edit mode is over
        flushSaveSelection();
      }
    }
  }
  ...
  private void updateDocumentEditState(boolean editing) {
    ...
    mutable().setEditingMode(editing);
  ...
  public void onJavaScriptEvent(String name, Event rawEvent) {
    ...
    boolean hackEditorNeverConsumes = name.equals(JsEvents.CONTEXT_MENU)
        || name.equals(JsEvents.CLICK)
        || name.equals(JsEvents.MOUSE_DOWN);
    ...
    try {
      ...
      // Stop propagation for all editor-relevant events (except mouse up)
      if (!hackEditorNeverConsumes) {
        if (editorRelevantEvent(event) && !name.equals(JsEvents.MOUSE_UP)) {
          try {
        ...
        try {
          if (debugDisabled) {
            ...
            if (!isMutationEvent) {
              ...
              boolean trackCursor = event.shouldTrackCursor();
