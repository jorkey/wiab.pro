...
public class TextSelectionController implements ToolbarToggleButton.Listener, Controller {
  ...
  private final boolean wholeParagraph;

  public TextSelectionController(ToolbarToggleButton button, EditorContext editor,
      String annotationName, String annotationValue, boolean wholeParagraph) {
    this.button = button;
    this.editor = editor;
    this.annotationName = annotationName;
    this.annotationValue = annotationValue;
    this.wholeParagraph = wholeParagraph;
  }
  ...
  private void setSelectionAnnotation(final String style) {
    if (editor.getSelectionHelper().getSelectionPoints() != null) {
      editor.undoableSequence(
          new Runnable() {
            
            @Override
            public void run() {
              EditorAnnotationUtil.setAnnotationOverSelection(
                  editor, annotationName, style, wholeParagraph);
            }
          });
    }
  }
  ...
  public void update(Range selectionRange) {
    if (annotationValue != null) {
      final String value =
          EditorAnnotationUtil.getAnnotationOverRangeIfFull(
          editor.getDocument(), editor.getCaretAnnotations(), annotationName,
