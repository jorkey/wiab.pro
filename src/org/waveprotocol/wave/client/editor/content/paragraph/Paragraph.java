...
public class Paragraph {
  ...
  /**
   * Extends the given selection range to the whole paragraph(s) size
   *
   * @param mapper - document
   * @param initRange - initial range
   * @return new range extended to the paragraph limits
   */
  public static Range extendRange(LocationMapper<ContentNode> mapper, Range initRange) {

    // go through the lines one by one:
    Line lineAt = getFirstLine(mapper, initRange.getStart());
    if (lineAt == null) {
      return new Range(0);
    }
