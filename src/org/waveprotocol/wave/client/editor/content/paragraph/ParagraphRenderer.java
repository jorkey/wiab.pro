...
public class ParagraphRenderer extends RenderingMutationHandler {
  ...
  private final IdentitySet<ContentElement> elementsToRender = CollectionUtils.createIdentitySet();
  ...
  private final Scheduler.Task batchRenderCommand = new Scheduler.Task() {
    ...
    private void render(ContentElement p) {
      ...
      htmlRenderer.updateRendering(p,
          ...
          indent,
          Paragraph.Alignment.fromValue(p.getAttribute(Paragraph.ALIGNMENT_ATTR)),
          Paragraph.Direction.fromValue(p.getAttribute(Paragraph.DIRECTION_ATTR)));
