...
public class CompositionEventHandler<V> {
  ...
  public boolean handleCompositionEvent(V event, String typeName) {
    if (JsEvents.COMPOSITION_START.equals(typeName)) {
      compositionStart(event);
    } else if (
        JsEvents.TEXT.equals(typeName) ||
        JsEvents.COMPOSITION_UPDATE.equals(typeName)) {
      compositionUpdate();
    } else if (JsEvents.COMPOSITION_END.equals(typeName)){
      compositionEnd();
    } else if (JsEvents.TEXT_INPUT.equals(typeName)) {
      textInput();
    } else {
