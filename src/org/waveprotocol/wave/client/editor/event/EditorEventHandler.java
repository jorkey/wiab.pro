...
public final class EditorEventHandler {
  ...
  private boolean handleInputOrDeleteKeyEvent(EditorEvent event, KeySignalType keySignalType)
    ...
    if (isCollapsed) {
      ...
      // NOTE: at this point, should be either INPUT or TEXT_DELETE
    ...
    if (keySignalType == KeySignalType.DELETE) {
      ...
      KeyCombo key = EventWrapper.getKeyCombo(event);
  ...
  private boolean isWhiteListedCombo(SignalEvent signal) {
    ...
    switch (keyCombo) {
      ...
      case CTRL_V: // paste
      case META_A:
      case CTRL_A: // select all
      case META_P:
      case CTRL_P: // print
      case META_L:
      case CTRL_L: // navigate to url box
      ...
      case META_HOME: // home
      case META_O:
      case CTRL_O: // open file
      case META_R:
      case CTRL_R: // reload
      case META_SHIFT_R:
      case CTRL_SHIFT_R: // reload (override cache)

      // Search
      case META_F:
      case CTRL_F: // find
      case META_G:
      case CTRL_G: // find again

      // tools
      case META_D:
      case CTRL_D: // bookmark this page

     // Window and tabs
      case META_N:
      case CTRL_N: // new window
      case META_T:
      case CTRL_T: // new tab
      case META_W:
      case CTRL_W: // close window
      case META_Q:
      case CTRL_Q: // quit
        return true;
