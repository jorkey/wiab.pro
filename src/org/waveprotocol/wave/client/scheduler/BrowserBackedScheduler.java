...
public class BrowserBackedScheduler implements Scheduler {
  ...
  public BrowserBackedScheduler() {
    this(GwtSimpleTimer.FACTORY, Controller.NOOP);
  }
    ...
    try {
      if (job instanceof IncrementalTask) {
        ...
        if (isFinished) {
          ...
          if (taskInfo != null && hasJob(job)) {
            // if it is a repeating job, add it to a delay before we contiune
            if (taskInfo.calculateNextExecuteTime(start)) {
              delayedJobs.addDelayedJob(taskInfo);
            } else if (!delayedJobs.has(taskInfo.id)) {
              jobs.add(priority, job);
            }
          }
        }
