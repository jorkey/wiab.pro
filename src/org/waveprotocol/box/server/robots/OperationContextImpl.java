...
public class OperationContextImpl implements OperationContext, OperationResults {
  ...
  public OpBasedWavelet openWavelet(WaveId waveId, WaveletId waveletId, ParticipantId participant)
    ...
    OpBasedWavelet opWavelet;
    if (wavelet == null) {
      // Open a wavelet from the server
      ReadableWaveletData snapshot = getWaveletSnapshot(waveletName, participant);
      if (snapshot == null) {
        if (waveletName.waveletId.equals(buildUserDataWaveletId(participant, waveId.getDomain()))) {
        ...
        opWavelet = wavelet.getOpBasedWavelet(participant);
        opWavelet.addParticipant(participant);
      } else {
        ObservableWaveletData obsWavelet = FACTORY.create(snapshot);
        wavelet = new RobotWaveletData(obsWavelet, snapshot.getHashedVersion());
        opWavelet = wavelet.getOpBasedWavelet(participant);
      }
    ...
    LOG.info("Opened wavelet " + waveletName.toString() + " in "
  ...
  @Override
  public ReadableWaveletData getWaveletSnapshot(WaveletName waveletName, ParticipantId participant) throws InvalidRequestException {
    try {
  ...
  @Override
  public ReadableWaveletData getWaveletSnapshot(WaveletName waveletName, HashedVersion version, ParticipantId participant)
    throws InvalidRequestException {
    try {
      if (!waveletProvider.checkExistence(waveletName)) {
        return null;
      ...
      return waveletProvider.getSnapshot(waveletName, version);
    } catch (WaveServerException ex) {
      LOG.severe("Error of access to wavelet " + waveletName, ex);
      return null;
    }
  ...
  public void getDeltas(WaveletName waveletName, ParticipantId participant,
      ...
      final ThrowableReceiver<WaveletDeltaRecord, InvalidRequestException> receiver)
      throws InvalidRequestException {
    try {
      if (!waveletProvider.checkExistence(waveletName)) {
        throw new InvalidRequestException("Wavelet not exists");
      }
      if (!waveletProvider.checkAccessPermission(waveletName, participant)) {
        throw new InvalidRequestException("Access rejected");
      }
      ...
      waveletProvider.getDeltaHistory(waveletName, fromVersion, toVersion, new ThrowableReceiver<WaveletDeltaRecord, WaveServerException>() {

        @Override
        public boolean put(WaveletDeltaRecord record) throws WaveServerException {
          try {
