...
import org.waveprotocol.wave.model.img.ImgXmlUtil;
...
public class DocumentModifyService implements OperationService {
  private static final Log LOG = Log.get(DocumentModifyService.class);
  
  private final AttachmentService attachmentService;
  private final AttachmentIdGeneratorImpl attachmentIdGenerator;

  @Inject
  public DocumentModifyService(AttachmentService attachmentService, IdGenerator idGenerator) {
  ...
  public void execute(
    ...
    Document doc = context.getBlip(conversation, blipId).getDocument();
      ...
      if (element != null) {
        if (element.isGadget()) {
          ...
          if (image.getUrl() != null) {
            XmlStringBuilder xml = ImgXmlUtil.constructXml(image.getUrl());
            LineContainers.appendLine(doc, xml);
          } else {
          throw new UnsupportedOperationException(
              "Can't insert other elements than ...");
          }
          ...
          try {
            if (attachment.getAttachmentId() != null) {
              attachmentId = AttachmentId.deserialise(attachment.getAttachmentId());
            } else {
              attachmentId = attachmentIdGenerator.newAttachmentId();
            }
            attachmentService.storeAttachment(attachmentId,
                new ByteArrayInputStream(attachment.getData()),
                WaveletName.of(view.getWavelet().getWaveId(), view.getWavelet().getId()),
          ...
          XmlStringBuilder xml = ImageXmlUtil.constructXml(attachmentId.serialise(), attachment.getCaption());
          LineContainers.appendLine(doc, xml);
        } else {
