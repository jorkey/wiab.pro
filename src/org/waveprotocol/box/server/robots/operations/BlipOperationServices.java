...
public class BlipOperationServices implements OperationService {
  private static final Log LOG = Log.get(BlipOperationServices.class);
    ...
    try {
      switch (type) {
        ...
        case BLIP_DELETE:
          ...
          break;
        case WAVELET_MODIFY_TAG:
          waveletModifyTag(operation, context, participant, conversation);
          break;
        default:
      ...
      LOG.severe("Blip getting error", ex);
      context.constructErrorResponse(operation, "Blip getting error");
    }
  }
  ...
  private void appendInlineBlip(OperationRequest operation, OperationContext context,
    ...
    Document doc = parentBlip.getDocument();
  ...
  private void appendMarkup(OperationRequest operation, OperationContext context,
    ...
    Document doc = convBlip.getDocument();
  ...
  /**
   * Implementation for the {@link OperationType#WAVELET_MODIFY_TAG} method. It deletes
   * the blip specified in the operation.
   *
   * @param operation the operation to execute.
   * @param context the context of the operation.
   ...
   * @throws InvalidRequestException if the operation fails to perform
   */
  private void waveletModifyTag(
      OperationRequest operation, OperationContext context,
      ParticipantId participant, ObservableConversation conversation)
      throws InvalidRequestException {
    Preconditions.checkArgument(
        OperationUtil.getOperationType(operation) == OperationType.WAVELET_MODIFY_TAG,
        "Unsupported operation " + operation);

    String name = OperationUtil.getRequiredParameter(operation, ParamsProperty.NAME);
    String modifyHow = OperationUtil.getRequiredParameter(operation, ParamsProperty.MODIFY_HOW);

    if ("add".equals(modifyHow)) {
      conversation.addTag(name);
    } else if ("remove".equals(modifyHow)) {
      conversation.removeTag(name);
    } else {
      throw new InvalidRequestException("Invalid parameter value", operation);
    }
  ...
  private void putContentForNewBlip(ConversationBlip newBlip, String content) {
    ...
    LineContainers.appendToLastLine(newBlip.getDocument(), builder);
  }
