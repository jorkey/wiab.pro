...
public final class DataApiTokenContainer {
  ...
  /** Number of minutes a request token is valid after last use */
  private static final int REQUEST_TOKEN_EXPIRATION = 10;

  /**
   * Number of minutes an access token is valid after last use, public so this can be shown to
   * the user
   */
  ...
  DataApiTokenContainer(TokenGenerator tokenGenerator) {
    ...
    requestTokenAccessors = CacheBuilder.newBuilder()
        .expireAfterAccess(REQUEST_TOKEN_EXPIRATION, TimeUnit.MINUTES)
        .<String, OAuthAccessor>build().asMap();
    accessTokenAccessors = CacheBuilder.newBuilder()
        .expireAfterAccess(ACCESS_TOKEN_EXPIRATION, TimeUnit.MINUTES)
        .<String, OAuthAccessor>build().asMap();
  }
