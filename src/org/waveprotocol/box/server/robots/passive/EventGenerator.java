...
public class EventGenerator {
  ...
  private class EventGeneratingConversationListener implements ObservableConversation.BlipListener,
      ObservableConversation.ParticipantListener, ObservableConversation.TagListener {
    ...
    public void onTagAdded(String tag, WaveletOperationContext opContext) {
      if (capabilities.containsKey(EventType.WAVELET_TAGS_CHANGED)) {
        tagsChanged = true;
      }
    }
    ...
    @Override
    public void onBeforeBlipRemoved(ObservableConversationBlip blip, WaveletOperationContext opContext) {}
    ...
    public void deltaEnd() {
      ...
      if (tagsChanged) {
        ...
        WaveletTagsChangedEvent event =
            new WaveletTagsChangedEvent(null, null, deltaAuthor.getAddress(),
                deltaTimestamp, rootBlipId);
        addEvent(event, capabilities, rootBlipId, messages);
      }
  ...
  private class EventGeneratingDocumentHandler implements DocHandler {
    ...
    @Override
    public void onDocumentEvents(EventBundle<N, E, T> event, WaveletOperationContext opContext) {
      ...
      for (DocumentEvent<N, E, T> eventComponent : eventComponents) {
        if (eventComponent.getType() == DocumentEvent.Type.ANNOTATION_CHANGED) {
          if (capabilities.containsKey(EventType.ANNOTATED_TEXT_CHANGED)) {
            ...
            AnnotatedTextChangedEvent apiEvent = new AnnotatedTextChangedEvent(
        ...
        } else if (eventComponent.getType() == DocumentEvent.Type.CONTENT_INSERTED) {
          if (capabilities.containsKey(EventType.GADGET_STATE_CHANGED)) {
            ContentInserted<N, E, T> contentInsertedEvent =
                (ContentInserted<N, E, T>) eventComponent;
            E element = contentInsertedEvent.getSubtreeElement();
            if (element != null) {
              if (GadgetConstants.STATE_TAGNAME.equals(doc.getTagName(element))) {
                E parentElement = doc.getParentElement(element);
                if (parentElement != null &&
                    GadgetConstants.TAGNAME.equals(doc.getTagName(parentElement))) {
                  int index = getGadgetIndex(parentElement);
                  if (index != -1) {
                    Map<String, String> oldState = getGadgeState(parentElement);
                    String key = doc.getAttribute(
                        element, GadgetConstants.KEY_ATTRIBUTE);
                    oldState.remove(key);
                    GadgetStateChangedEvent apiEvent =
                        new GadgetStateChangedEvent(
                        null, null, deltaAuthor.getAddress(), deltaTimestamp,
                        blip.getId(), index, oldState);
                    addEvent(apiEvent, capabilities, blip.getId(), messages);
                  }
                }
              }
        ...
        } else if (eventComponent.getType() == DocumentEvent.Type.ATTRIBUTES) {
          if (capabilities.containsKey(EventType.GADGET_STATE_CHANGED)) {
            AttributesModified<N, E, T> attributesModifiedEvent =
                (AttributesModified<N, E, T>) eventComponent;

            E element = attributesModifiedEvent.getElement();
            if (element != null) {
              if (GadgetConstants.STATE_TAGNAME.equals(doc.getTagName(element))) {
                E parentElement = doc.getParentElement(element);
                if (parentElement != null &&
                  ...
                  if (index != -1) {
                    ...
                    Map<String, String> oldValues =
                        attributesModifiedEvent.getOldValues();
                    if (oldValues.get(GadgetConstants.VALUE_ATTRIBUTE) != null) {
                      String key = doc.getAttribute(element, GadgetConstants.KEY_ATTRIBUTE);
                      oldState.put(key, oldValues.get(GadgetConstants.VALUE_ATTRIBUTE));
                    }
                    GadgetStateChangedEvent apiEvent =
                        new GadgetStateChangedEvent(null, null, deltaAuthor.getAddress(), deltaTimestamp,
                        blip.getId(), index, oldState);
                    addEvent(apiEvent, capabilities, blip.getId(), messages);
                  } else {
                    LOG.severe("Can't find event component in the blip");
                  }
                }
              }
            }
        ...
        if (capabilities.containsKey(EventType.DOCUMENT_CHANGED) && !documentChangedEventGenerated) {
    ...
    private int getGadgetIndex(E gadgetElement) {
      E bodyElement = Blips.getBody(blip.getDocument());
      if (bodyElement != null) {
        ...
        while (child != null) {
          ...
          if (asText != null) {
            index += doc.getData(asText).length();
          } else {
            E xmlElement = doc.asElement(child);
            if (xmlElement == gadgetElement) {
              return index;
            }
            if (xmlElement != null) {
              index++;
    ...
    private Map<String, String> getGadgeState(E gadgetElement) {
      Map<String, String> state = new HashMap<>();
      for (N n = doc.getFirstChild(gadgetElement); n != null; n = doc.getNextSibling(n)) {
        E e = doc.asElement(n);
        if (GadgetConstants.STATE_TAGNAME.equals(doc.getTagName(e))) {
          String key = doc.getAttribute(e, GadgetConstants.KEY_ATTRIBUTE);
          String value = doc.getAttribute(e, GadgetConstants.VALUE_ATTRIBUTE);
          state.put(key, value);
        }
      }
      return state;
  ...
  public EventMessageBundle generateEvents(WaveletAndDeltas waveletAndDeltas,
    ...
    conversation.addBlipListener(conversationListener);
    conversation.addParticipantListener(conversationListener);
    conversation.addTagListener(conversationListener);
    ...
    try {
      ...
      conversation.removeBlipListener(conversationListener);
      conversation.removeParticipantListener(conversationListener);
      conversation.removeTagListener(conversationListener);
