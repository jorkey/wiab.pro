...
public abstract class AbstractWaveSocketIOServlet extends HttpServlet {
  ...
  private static final Logger LOGGER = Logger.getLogger(
      SocketIOServlet.class.getName());
  private static final long serialVersionUID = 2L;
  private final SocketIOSessionManager sessionManager =
      new SocketIOSessionManager();
  private final TransportHandlerProvider transportHandlerProvider =
      new AnnotationTransportHandlerProvider();
  ...
  public void init() throws ServletException {
    ...
    if (LOGGER.isLoggable(Level.INFO)) {
      LOGGER.log(Level.INFO, "Transport handlers loaded: " +
          transportHandlerProvider.listAll());
    }
    ...
    for (Transport transport : transports) {
      if (transportHandlerProvider.isSupported(transport.getType())) {
        ...
        LOGGER.log(Level.WARNING, "Transport " + transport.getType() +
            " ignored since not supported by any TransportHandler");
      }
    }
    // initialize them
    for (Transport t : config.getTransports()) {
      try {
        ...
        LOGGER.log(Level.WARNING, "Transport " + t.getType() +
            " disabled. Initialization failed: " + e.getMessage());
      }
    }
    if (LOGGER.isLoggable(Level.INFO)) {
  ...
  /**
   * Returns an instance of SocketIOInbound or null if the connection is to be
   * denied. The value of cookies and protocols may be null.
   */
  ...
  private void serve(HttpServletRequest request, HttpServletResponse response)
    ...
    if (transport == null) {
      if ("GET".equals(request.getMethod()) && "socket.io.js".equals(parts[0])) {
        ...
        InputStream is = this.getClass().getClassLoader().getResourceAsStream(
            "com/glines/socketio/socket.io.js");
        OutputStream os = response.getOutputStream();
        IO.copy(is, os);
        return;
      } else if ("GET".equals(request.getMethod()) && 
          "WebSocketMain.swf".equals(parts[0])) {
        response.setContentType("application/x-shockwave-flash");
        InputStream is = this.getClass().getClassLoader().getResourceAsStream(
            "com/glines/socketio/WebSocketMain.swf");
        OutputStream os = response.getOutputStream();
        IO.copy(is, os);
        return;
      } else {
    ...
    if (LOGGER.isLoggable(Level.FINE)) {
      LOGGER.log(Level.FINE, "Handling request from " + request.getRemoteHost()
          + ":" + request.getRemotePort() + " with transport: " + transport.getType());
    }
