...
public final class ModelIdMapperImpl implements ModelIdMapper {
  ...
  private final static JavaCodec CODEC = new JavaCodec();

  /** Component separator, as provided by StringCodec. */
  private final static String SEP = ",";

  /** Model context in which this mapper is mapping. */
  private final org.waveprotocol.wave.model.conversation.ConversationView model;
  ...
  @VisibleForTesting
  ModelIdMapperImpl(org.waveprotocol.wave.model.conversation.ConversationView model, String prefix, int nextId) {
    this.model = model;
    this.prefix = prefix;
    this.nextId = nextId;
  }
   ...
   */
  public static ModelIdMapperImpl create(org.waveprotocol.wave.model.conversation.ConversationView model, String prefix) {
   ...
   */
  public static ModelIdMapperImpl create(org.waveprotocol.wave.model.conversation.ConversationView model, String prefix, int nextId) {
  ...
  @Override
  public String tagId(Conversation c, String t) {
    return shorten(longConvId(c) + SEP + CODEC.encode(t));
  }  
  ...
  @Override
  public Pair<Conversation, String> locateTag(String modelId) {
    String longId = restoreId(modelId);
    String[] parts = split(longId);

    if (parts.length != 2) {
      throw new IllegalArgumentException("Not a tag model id: " + modelId);
    } else {
      Conversation c = model.getConversation(parts[0]);
      return Pair.of(c, parts[1]);
    }
  } 
  ...
  public void registerIdPair(String shortId, String longId) {
    Preconditions.checkArgument(!shortIdToLongIdMap.containsKey(shortId) &&
        !longIdToShortIdMap.containsKey(longId), "Id already exists");
