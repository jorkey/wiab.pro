...
package org.waveprotocol.box.server.rpc.render.account.impl;
...
//      fullName = Joiner.on(' ').join(names);
  ...
  public static String coverName(String name) {
    ...
    String nameWithoutDomain = parts[0];
    String domain = parts.length>1?parts[1]:null;
    if (!AccountStoreHolder.getDefaultDomain().equals(domain)) {
      if (nameWithoutDomain != null && !nameWithoutDomain.isEmpty()) {
        if (nameWithoutDomain.length() > 3) {
          nameWithoutDomain = nameWithoutDomain.substring(0, 3);
        }
         nameWithoutDomain += "***";
        return domain != null ? nameWithoutDomain + "@" + domain : nameWithoutDomain;
      }
    }
    return name;
  }
