...
public final class BlipReadStateMonitorImpl extends ObservableSupplementedWave.ListenerImpl
  ...
  private static final Logger LOG =
      Logger.getLogger(BlipReadStateMonitorImpl.class.getName());
  ...
  private final IdentitySet<ConversationBlip> unreadBlips =
      CollectionUtils.createIdentitySet();
  // IdentitySet has no O(1) size() method, so sizes must be maintained manually.

  private int read;
  private int unread;
  private final CopyOnWriteSet<BlipReadStateMonitor.Listener> listeners =
   ...
   */
  public static BlipReadStateMonitorImpl create(
      WaveId waveId, ObservableSupplementedWave supplementedWave,
      ObservableConversationView conversationView) {
    BlipReadStateMonitorImpl monitor = new BlipReadStateMonitorImpl(
        waveId, supplementedWave, conversationView);
    monitor.init();
    return monitor;
  }

  private BlipReadStateMonitorImpl(
      WaveId waveId, ObservableSupplementedWave supplementedWave,
      ObservableConversationView conversationView) {
  ...
  private void notifyListeners() {
    LOG.fine(Joiner.on(",").join(
        waveId, ": notifying read/unread change ", read, "/", unread));
  ...
  private void logChange(String action, ConversationBlip blip) {
    LOG.fine(Joiner.on(",").join(
        blip, ": ", action, " now ", getReadCount(), "/", getUnreadCount()));
  }
