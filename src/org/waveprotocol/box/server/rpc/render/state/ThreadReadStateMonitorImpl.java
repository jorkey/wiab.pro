...
package org.waveprotocol.box.server.rpc.render.state;
 ...
 *
 * Counts are lazily initialized such that the thread state will not be counted
 * nor tracked unless the thread count has been explicitly queried.  This allows
 * for nice optimizations such as not counting root threads (if this monitor is
 * only used for inline reply counts), paging of threads, etc.
 */
public final class ThreadReadStateMonitorImpl
  ...
  private class ReadStateBucket {
    ...
    private final IdentitySet<ConversationBlip> readBlips =
        CollectionUtils.createIdentitySet();
    private final IdentitySet<ConversationBlip> unreadBlips =
        CollectionUtils.createIdentitySet();
    ...
    public boolean handleWaveletReadStateChanged() {
      ...
      final IdentitySet<ConversationBlip> changedBlips =
          CollectionUtils.createIdentitySet();

      readBlips.each(
          new IdentitySet.Proc<ConversationBlip>() {
   ...
   */
  private final IdentitySet<ConversationThread> eventThreads =
      CollectionUtils.createIdentitySet();
   ...
   */
  public static ThreadReadStateMonitorImpl create(
  ...
  private ThreadReadStateMonitorImpl(
      ObservableSupplementedWave supplementedWave,
      ObservableConversationView conversationView) {
  ...
  private void init() {
    ...
    if (!threadsToNotifyWhenReady.isEmpty()) {
      ...
      threadsToNotifyWhenReady.each(
          new IdentitySet.Proc<ConversationThread>() {
  ...
  public int getReadCount(ConversationThread thread) {
    Preconditions.checkState(
        isReady, "ThreadReadStateMonitor queried before ready");
  ...
  @Override
  public void onReplyAdded(ObservableConversationThread thread, WaveletOperationContext opContext) {
  ...
  @Override
  public void onBeforeReplyRemoved(ObservableConversationThread thread, WaveletOperationContext opContext) {
  }

  @Override
  public void onReplyRemoved(ObservableConversationThread thread, WaveletOperationContext opContext) {
  ...
  public void onMaybeWaveletReadChanged(ObservableConversation conversation) {
    ...
    // recalculated.
    threadStates.each(
        new IdentityMap.ProcV<ConversationThread, ThreadReadState>() {
...
//    assert supplement.isReady();
  ...
  private ConversationThread getParent(ConversationThread thread) {
    return (thread.getParentBlip() != null) ?
        thread.getParentBlip().getThread() : null;
  }

  /**
   * @return the parent thread of a thread, or null if the thread is top-level
   */
  private ObservableConversationThread getParent(
      ObservableConversationThread thread) {
    return thread.getParentBlip() != null ?
        thread.getParentBlip().getThread() : null;
  }
...
//  @Override
//  public void onInlineThreadAdded(
//      ObservableConversationThread thread, int location) {
//  }
