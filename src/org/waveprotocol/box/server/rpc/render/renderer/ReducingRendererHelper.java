...
public final class ReducingRendererHelper<R> implements org.waveprotocol.box.server.rpc.render.renderer.ResultProducingRenderHelper<R> {
  ...
  public void endView(ConversationView wave) {
    ...
    currentScope().add(wave, builders.renderConversations(wave, convs));
  }
  ...
  public void endConversation(Conversation conv) {
    ...
    for (ParticipantId participant : conv.getParticipantIds()) {
      allParticipants.put(participant.getAddress(),
          builders.renderParticipant(conv, participant));
    }    
    R participants = builders.renderParticipants(conv, allParticipants);
    
    StringMap<R> allTags = CollectionUtils.createStringMap();
    for (String tag : conv.getTags()) {
      allTags.put(tag, builders.renderTag(conv, tag));
    }    
    R tags = builders.renderTags(conv, allTags);
        
    R rootThread = leave().getThreads().get(conv.getRootThread());    
    currentScope().add(conv,
        builders.renderConversation(conv, participants, rootThread, tags));
  }
  ...
  public void endBlip(ConversationBlip blip) {
    ...
    R document = builders.renderNamedDocument(blip, threads);
    ...
    for (ConversationThread reply : blip.getReplyThreads()) {
      defaultAnchors.put(reply, builders.renderDefaultAnchor(reply, threads.get(reply)));
    }

    currentScope().add(blip, builders.renderBlip(blip, document, defaultAnchors, nestedConversations));
  }
  ...
  public void endThread(ConversationThread thread) {
    ...
    currentScope().add(thread, builders.renderThread(thread, blips));
  }
