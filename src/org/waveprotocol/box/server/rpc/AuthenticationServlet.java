...
public class AuthenticationServlet extends HttpServlet {
  ...
  private LoginContext login(BufferedReader body) throws IOException, LoginException {
    try {
      ...
      UrlEncoded parameters = new UrlEncoded();
      parameters.decode(parametersLine);
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    RegistrationUtil.createGreetingIfNotExists(loggedInAddress, welcomeBot);
          ...
          if (email.endsWith("@" + clientAuthCertDomain)) {
            ...
            if (!RegistrationUtil.doesAccountExist(accountStore, id)) {
              if (!isRegistrationDisabled) {
                if (!RegistrationUtil.createAccountAndGreeting(accountStore, id, null, welcomeBot)) {
                  return null;
                }
              } else {
  ...
  private void redirectLoggedInUser(HttpServletRequest req, HttpServletResponse resp)
    ...
    String locale = null;
    AccountData account = sessionManager.getLoggedInAccount(req.getSession(false));
    if (account != null) {
      locale = account.asHuman().getLocale();
    }
    ...
    if (query == null || !query.startsWith("r=")) {
      resp.sendRedirect(addLocale(DEFAULT_REDIRECT_URL, locale));
      return;
    }
    ...
    if (Strings.isNullOrEmpty(uri.getHost()) == false) {
      ...
      resp.sendRedirect(addLocale(path, locale));
    }
  }

  private String addLocale(String url, String locale) {
    if (locale != null) {
      return UrlParameters.addParameter(url, "locale", locale);
    }
    return url;
  }
}
