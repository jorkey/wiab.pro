...
import org.waveprotocol.wave.clientserver.ClientServer.DisconnectRequest;
...
public class ServerRpcProvider {
  ...
  static class WebSocketConnection extends Connection {
    ...
    @Inject
    WebSocketConnection(ServerRpcProvider provider) {
      super(provider);
  ...
  static class SocketIOConnection extends Connection {
    ...
    @Inject
    SocketIOConnection(ServerRpcProvider provider) {
      super(provider);
  ...
  static abstract class Connection implements ProtoCallback {
     ...
     * @param provider
     */
    public Connection(ServerRpcProvider provider) {
    ...
    @Override
    public void message(final int sequenceNo, Message message, String connectionId) {
      processMessage(sequenceNo, message, connectionId);
    }

    void processMessage(final int sequenceNo, Message message, String connectionId) {
      ...
      if (message instanceof Rpc.CancelRpc) {
      ...
      } else if (message instanceof TransportAuthenticationRequest) {
        ...
        TransportAuthenticationRequest authMessage = (TransportAuthenticationRequest) message;
        ParticipantId authenticatedAs = authenticate(authMessage.getToken());

        if (loggedInUser != null) {
          // Check that session already authenticated as a different user.
          if (!loggedInUser.equals(authenticatedAs)) {
            loggedInUser = null;
          }
        } else {
        ...
        if (activeRpcs.containsKey(sequenceNo)) {
          ...
          final ServerRpcController controller =
              new ServerRpcControllerImpl(message, serviceMethod.service, serviceMethod.method,
                  loggedInUser, connectionId, new RpcCallback<Message>() {
                    @Override
    ...
    public void disconnect(String connectionId) {
      Message message = DisconnectRequest.getDefaultInstance();
      final RegisteredServiceMethod serviceMethod =
          provider.registeredServices.get(message.getDescriptorForType());
      final ServerRpcController controller =
          new ServerRpcControllerImpl(message, serviceMethod.service, serviceMethod.method,
  ...
  public ServerRpcProvider(InetSocketAddress[] httpAddresses, Integer flashsocketPolicyPort,
    ...
    this(httpAddresses, flashsocketPolicyPort, resourceBases, executorService,
  ...
  public ServerRpcProvider(@Named(CoreSettings.HTTP_FRONTEND_ADDRESSES) List<String> httpAddresses,
      ...
      @ExecutorAnnotations.ClientServerExecutor Executor executorService) {
  ...
  public static class WaveWebSocketServlet extends WebSocketServlet {
    ...
    final Provider<MultiThreadedRequestScope> scope;
    final Provider<WebSocketConnection> webSocketConnection;
    ...
    public WaveWebSocketServlet(ServerRpcProvider provider,
        ...
        Provider<MultiThreadedRequestScope> scope,
        Provider<WebSocketConnection> webSocketConnection) {
      super();
      this.provider = provider;
  ...
  public static class WaveSocketIOServlet extends HttpServlet {
    ...
    private final ServerRpcProvider provider;
    private final Provider<SocketIOConnection> socketIOConnection;

    @Inject
    public WaveSocketIOServlet(ServerRpcProvider provider, Provider<SocketIOConnection> socketIOConnection) {
      this.provider = provider;
    ...
    AbstractWaveSocketIOServlet socketIOServlet = new AbstractWaveSocketIOServlet( new Transport[] {
        ...
        // TODO(akaplanov): restore JettyWebSocketTransport when it will updated to Jetty v9.
        new JSONPPollingTransport(), new HTMLFileTransport()}) {
      @Override
      protected SocketIOInbound doSocketIOConnect(final HttpServletRequest req) {
        return socketIOConnection.get().getWebSocketServerChannel();
      }
    };
  ...
  public void stopServer() throws IOException {
    ...
    try {
      ...
      LOG.info("Jetty server is stopped.");
    } catch (Exception e) {
  ...
  public ServletHolder addServlet(String urlPattern, Class<? extends HttpServlet> servlet,
    ...
    servletRegistry.add(new Pair<>(urlPattern, servletHolder));
