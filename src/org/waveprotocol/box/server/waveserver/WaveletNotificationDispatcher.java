...
class WaveletNotificationDispatcher implements WaveBus, WaveletNotificationSubscriber {
  ...
  @Override
  public void waveletUpdate(WaveletName waveletName, ImmutableList<WaveletDeltaRecord> deltas,
    ...
    for (WaveBus.Subscriber s : subscribers) {
      try {
        s.waveletUpdate(waveletName, sequence);
      } catch (Exception e) {
        LOG.severe("Exception in update to wave bus subscriber " + s, e);
      }
    }
    ...
    if (!remoteDomainsToNotify.isEmpty()) {
      ...
      for (String domain : remoteDomainsToNotify) {
        try {
          federationHosts.get(domain).waveletDeltaUpdate(waveletName,
  ...
  public void waveletCommitted(WaveletName waveletName, HashedVersion version,
    ...
    Set<String> remoteDomainsToNotify = Sets.difference(domainsToNotify, localDomains);
    if (!remoteDomainsToNotify.isEmpty()) {
      ProtocolHashedVersion serializedVersion = OperationSerializer.serialize(version);
      for (String domain : remoteDomainsToNotify) {
        try {
