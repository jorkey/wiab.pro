...
package org.waveprotocol.box.server.serialize;
...
public class OperationSerializer {
  ...
  public static ProtocolDocumentOperation serialize(DocOp inputOp) {
    ...
    inputOp.apply(new DocOpCursor() {
      ...
      public void replaceAttributes(Attributes oldAttributes, Attributes newAttributes) {
        ...
        if (oldAttributes.isEmpty() && newAttributes.isEmpty()) {
          ...
          for (String name : oldAttributes.keySet()) {
            r.addOldAttribute(
                ProtocolDocumentOperation.Component.KeyValuePair.newBuilder()
        ...
        output.addComponent(newComponentBuilder().setReplaceAttributes(
            r.build()).build());
      }
      ...
      public void updateAttributes(AttributesUpdate attributes) {
        ...
        output.addComponent(newComponentBuilder().setUpdateAttributes(
            u.build()).build());
      }
  ...
  public static WaveletDelta deserialize(ProtocolWaveletDelta delta) {
    ...
    int opCount = delta.getOperationCount();
    Preconditions.checkArgument(opCount > 0, "Cannot deserialize an empty delta");
    for (int i=0; i < opCount; i++) {
      WaveletOperationContext context;
      if (i == opCount - 1) {
        context = new WaveletOperationContext(author, Constants.NO_TIMESTAMP, hashedVersion.getVersion(), hashedVersion);
      } else {
        context = new WaveletOperationContext(author, Constants.NO_TIMESTAMP, hashedVersion.getVersion());
      }
      ops.add(deserialize(delta.getOperation(i), context));
    }
  ...
  public static TransformedWaveletDelta deserialize(ProtocolWaveletDelta delta,
    ...
    for (int i=0; i < opCount; i++) {
      WaveletOperationContext context;
      if (i == opCount - 1) {
        context = new WaveletOperationContext(author, applicationTimestamp, resultingVersion.getVersion(), resultingVersion);
      } else {
  ...
  public static DocOp deserialize(ProtocolDocumentOperation op) {
    ...
    for (ProtocolDocumentOperation.Component c : op.getComponentList()) {
      if (c.hasAnnotationBoundary()) {
        ...
        output.elementStart(c.getElementStart().getType(),
