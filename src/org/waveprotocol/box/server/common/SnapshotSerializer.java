...
import org.waveprotocol.box.server.persistence.protos.ProtoSnapshotStoreData.DocumentSnapshot;
import org.waveprotocol.box.server.persistence.protos.ProtoSnapshotStoreData.WaveletSnapshot;
...
public class SnapshotSerializer {
  ...
  public static WaveletSnapshot serializeWavelet(ReadableWaveletData wavelet) {
    ...
    for (String id : wavelet.getDocumentIds()) {
      ReadableBlipData data = wavelet.getBlip(id);
      builder.addDocument(serializeDocument(data));
    }

    builder.setVersion(CoreWaveletOperationSerializer.serialize(wavelet.getHashedVersion()));
  ...
  public static ObservableWaveletData deserializeWavelet(WaveletSnapshot snapshot, WaveId waveId)
    ...
    for (String participant : snapshot.getParticipantIdList()) {
      wavelet.addParticipant(getParticipantId(participant), null);
    }
  ...
  public static DocumentSnapshot serializeDocument(ReadableBlipData document) {
    ...
    builder.setCreationTime(document.getCreationTime());
    builder.setCreationVersion(document.getCreationVersion());
