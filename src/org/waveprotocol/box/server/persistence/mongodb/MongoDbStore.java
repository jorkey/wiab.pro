...
public final class MongoDbStore implements SignerInfoStore, AttachmentStore, AccountStore {
  ...
  private DBObject humanToObject(HumanAccountData account) {
    ...
    if (account.getSearches() != null) {
      Searches searches = serializeSearches(account.getSearches());
      ByteArrayOutputStream out = new ByteArrayOutputStream();
      try {
        searches.writeTo(out);
        object.put(SEARCHES_FIELD, out.toByteArray());
      } catch (IOException ex) {
        LOG.log(Level.SEVERE, "Searches serialization error", ex);
      }
    }
    return object;
  }
  ...
  private Searches serializeSearches(List<SearchesItem> searches) {
    Searches.Builder searchesBuilder = Searches.newBuilder();
  ...
  private HumanAccountData objectToHuman(ParticipantId id, DBObject object) {
    ...
    if (digestObj != null) {
      ...
      PasswordDigest passwordDigest = PasswordDigest.from(salt, digest);
      account.setPasswordDigest(passwordDigest);
    }
    byte[] searches = (byte[])object.get(SEARCHES_FIELD);
    if (searches != null) {
      try {
        account.setSearches(deserializeSearches(Searches.parseFrom(searches)));
      } catch (InvalidProtocolBufferException ex) {
        LOG.log(Level.SEVERE, "Searches parse error", ex);
      }
    }
    return account;
  }
