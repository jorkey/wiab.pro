...
public class DeltaIndex {
   ...
   * @throws IOException
   */
  public void openForCollection(FileDeltaAccess baseCollection) throws IOException {
    Timer timer = Timing.start("DeltaIndex.openForCollection");
    try {
      if (!fileRef.exists()) {
        fileRef.mkdirs();
   ...
   * @throws IOException
   */
  public void rebuildIndexFromDeltas(FileDeltaAccess collection) throws IOException {
    Timer timer = Timing.start("DeltaIndex.rebuildIndexFromDeltas");
    try {
      if (file != null) {
        file.close();
      }
  ...
  /**
   * Get the delta file offset for the arbitrary version.
   *
   * @param version
   * @return the offset on success, NO_RECORD_FOR_VERSION if there's no record.
   * @throws IOException
   */
  public long getOffsetForArbitraryVersion(long version) throws IOException {
    if (!seekToPosition(version)) {
      return NO_RECORD_FOR_VERSION;
    }
    long offset = file.readLong();
  ...
  public void addDelta(long version, int numOperations, long offset)
      throws IOException {
    Timer timer = Timing.start("DeltaIndex.addDelta");
    try {
      checkOpen();
  ...
  private boolean seekToPosition(long version) throws IOException {
    Timer timer = Timing.start("DeltaIndex.seekToPosition");
    try {
      if (version < 0) {
        return false;
      }
