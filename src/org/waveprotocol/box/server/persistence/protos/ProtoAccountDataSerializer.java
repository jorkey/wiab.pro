...
import org.waveprotocol.box.searches.proto.SearchesItemProtoImpl;
...
public class ProtoAccountDataSerializer {
  ...
  private static ProtoHumanAccountData serialize(HumanAccountData account) {
    ...
    if (account.getSearches() != null) {
      builder.setSearches(serialize(account.getSearches()));
    }
  ...
  private static Searches serialize(List<SearchesItem> searches) {
    Searches.Builder searchesBuilder = SearchesProto.Searches.newBuilder();
    for (SearchesItem search : searches) {
      searchesBuilder.addSearch(new SearchesItemProtoImpl(search).getPB());
    }
  ...
  private static HumanAccountData deserialize(String accountId, ProtoHumanAccountData data) {
    ParticipantId id = ParticipantId.ofUnsafe(accountId);
    HumanAccountData account = new HumanAccountDataImpl(id);
    if (data.hasPasswordDigest()) {
      account.setPasswordDigest(deserialize(data.getPasswordDigest()));
    }
    if (data.hasSearches()) {
      account.setSearches(deserialize(data.getSearches()));
    }
    if (data.hasLocale()) {
      account.setLocale(data.getLocale());
    }
    return account;
  }
  ...
  private static List<SearchesItem> deserialize(SearchesProto.Searches searches) {
    List<SearchesItem> list = Lists.newArrayList();
    for (SearchesProto.SearchesItem protoSearch : searches.getSearchList()) {
      SearchesItem search = new SearchesItemImpl();
      search.copyFrom(new SearchesItemProtoImpl(protoSearch));
      list.add(search);
    }
    return list;
  }

  private static RobotAccountData deserialize(String accountId, ProtoRobotAccountData data) {
    ParticipantId id = ParticipantId.ofUnsafe(accountId);
