...
public abstract class ElementSerializer {
  ...
  private static ApiAttachment.Provider attachmentMedatataProvider;

  public static void setAttachmentDataProvider(ApiAttachment.Provider attachmentMedatataProvider) {
    ElementSerializer.attachmentMedatataProvider = attachmentMedatataProvider;
  }
  ...
  public static Element xmlToApiElement(Document doc, Doc.E element) {
    if (element == null) {
      return null;
    }
    ...
    return serializer.fromXml(doc, element);
  }
  ...
  static {
    register(new ElementSerializer("label", ElementType.LABEL) {
      ...
      @Override
      protected Element fromXml(Document doc, E element) {
    ...
    register(new ElementSerializer("image", ElementType.ATTACHMENT) {
      ...
      protected Element fromXml(Document doc, E element) {
        ...
        if (ElementSerializer.attachmentMedatataProvider != null && attachmentId != null) {
          ApiAttachment attachmentMetadata = null;
          try {
            attachmentMetadata = ElementSerializer.attachmentMedatataProvider.getAttachment(attachmentId);
          } catch (Exception ex) {
            LOG.log(Level.SEVERE, "Get attachment", ex);
          }
          if (attachmentMetadata != null) {
            properties.put(Attachment.MIME_TYPE, attachmentMetadata.getMimeType());
            properties.put(Attachment.FILE_NAME, attachmentMetadata.getFileName());
          }
        }
  ...
  protected abstract Element fromXml(Document doc, E element);
