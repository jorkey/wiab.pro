...
public class WaveService {
  ...
  private List<JsonRpcResponse> makeRpc(OperationQueue opQueue, String rpcServerUrl)
    ...
    try {
      ...
      try {
        bodyStream = new ByteArrayInputStream(json.getBytes(UTF_8));
      } catch (UnsupportedEncodingException e) {
        throw new IllegalStateException(e);
      }
      if (!consumerDataObj.isUserAuthenticated()) {
        String url = createOAuthUrlString(POST, json,
            consumerDataObj.getRpcServerUrl(), consumerDataObj.getAccessor(),
        ...
        HttpResponseMessage response = httpFetcher.execute(request, Collections.<String, Object>emptyMap());
        if (response.getStatusCode() != HttpResponse.STATUS_OK) {
          throw new IOException("Error HTTP response code " + response.getStatusCode());
        }
  ...
  public void fetchAttachment(String rpcServerUrl, Attachment attachment) throws IOException {
    InputStream input = makeGet(rpcServerUrl, "attachment", attachment.getAttachmentId());
  ...
  protected InputStream makeGet(String rpcServerUrl, String param, String value) throws IOException {
    try {
      if (rpcServerUrl == null) {
        throw new IllegalStateException("RPC Server URL is not set up.");
      }
      ...
      params.add(new SimpleEntry<String, String>(param, value));
      if (!consumerDataObj.isUserAuthenticated()) {
        String url;
        try {
          url = createOAuthUrlString(GET, null, consumerDataObj.getRpcServerUrl(),
              consumerDataObj.getAccessor(), params);
        } catch (URISyntaxException ex) {
          throw new IOException(ex);
        } catch (OAuthException ex) {
          throw new IOException(ex);
        }
        HttpMessage request = new HttpMessage(GET, new URL(url));
        request.headers.add(
            new SimpleEntry<String, String>(HttpMessage.CONTENT_TYPE, JSON_MIME_TYPE));
        request.headers.add(new SimpleEntry<String, String>("oauth_version", "1.0"));
        return httpFetcher.execute(request, Collections.<String, Object>emptyMap()).getBody();
      } else {
        OAuthAccessor accessor = consumerDataObj.getAccessor();
        OAuthMessage message = accessor.newRequestMessage(GET, rpcServerUrl, params);
        message.getHeaders().add(
            new SimpleEntry<String, String>(HttpMessage.CONTENT_TYPE, JSON_MIME_TYPE));
        message.getHeaders().add(new SimpleEntry<String, String>("oauth_version", "1.0"));
        OAuthClient client = new OAuthClient(httpFetcher);
