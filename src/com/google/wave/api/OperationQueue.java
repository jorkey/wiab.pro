...
public class OperationQueue implements Serializable {
  ...
  /**
   * Performs folder operation.
   *
   * @param wavelet the wavelet to modify the folder.
   * @param modifyHow how to modify the folder.
   *     Specify: {@code markAsRead} to mark as read,
   *              {@code markAsUnread} to mark as unread,
   *              {@code archive} to archive,
   *              {@code inbox} to move to inbox.
   * @param blipId blip for mark as read or unread. Specify null to perform for all wavelet.
   */
  public void folderAction(Wavelet wavelet, String modifyHow, String blipId) {
    Collection<Parameter> parameters = new ArrayList<Parameter>();
    parameters.add(Parameter.of(ParamsProperty.MODIFY_HOW, modifyHow));
    if (blipId != null) {
      parameters.add(Parameter.of(ParamsProperty.BLIP_ID, blipId));
    }
    appendOperation(OperationType.ROBOT_FOLDER_ACTION, wavelet,
        parameters.toArray(new Parameter[parameters.size()]));
  }

  /**
   * Performs folder operation for all participants.
   *
   * @param wavelet the wavelet to modify the folder.
   * @param modifyHow how to modify the folder.
   *     Specify: {@code archive} to archive,
   *              {@code inbox} to move to inbox.
   */
  public void folderActionForAll(Wavelet wavelet, String modifyHow) {
    appendOperation(OperationType.ROBOT_FOLDER_ACTION_FOR_ALL, wavelet,
        Parameter.of(ParamsProperty.MODIFY_HOW, modifyHow));
  }
