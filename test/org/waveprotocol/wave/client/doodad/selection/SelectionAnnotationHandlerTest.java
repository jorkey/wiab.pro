 ...
 */
public class SelectionAnnotationHandlerTest
    extends TestCase {

  public static class FakeCaretViewFactory
      implements CaretViewFactory {
    
    private final Map<CaretView, Object> markers =
        new HashMap<CaretView, Object>();
  ...
  protected void setUp() {
    Registries registries = new RegistriesImpl(
        mock(ElementHandlerRegistry.class), annotationRegistry, painterRegistry);
    ...
    cxt = ContextProviders.createTestPojoContext2(
        content, null, annotationRegistry, null,
  ...
  public void testBehaviour() {
    ...
    me.extractor.writeSelection(
        cxt.document(), new FocusedRange(2, 4), "wo",
        handlerTimer.currentTimeMillis());
    assertTrue(markerFactory.markers.isEmpty());
    check("abcdefghij");

    joe.extractor.writeSelection(
        cxt.document(), new FocusedRange(1, 3), "ni",
        handlerTimer.currentTimeMillis());
    ...
    joe.extractor.writeSelection(
        cxt.document(), new FocusedRange(2, 4), "ni",
        handlerTimer.currentTimeMillis());
  ...
  private void check(String xml) {
    assertTrue(DocCompare.equivalent(STRUCTURE - ATTRS,
        xml.replaceAll("<b/>", "<b><" + CaretMarkerRenderer.FULL_TAGNAME
        + "/></b>"),
