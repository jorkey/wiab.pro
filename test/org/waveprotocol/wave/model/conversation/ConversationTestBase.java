...
public abstract class ConversationTestBase extends TestCase {
  ...
  public void testInsertBlipBeforeFirstBlipCreatesNewFirstBlip() {
    ConversationThread thread = target.getRootThread();
    ConversationBlip oldFirst = thread.appendBlip();
    ConversationBlip newFirst = thread.insertBlip(oldFirst, true);
  ...
  public void testInsertBlipBetweenBlipsInserts() {
    ...
    ConversationBlip middle = thread.insertBlip(last, true);
  ...
  public void testAppendInlineReplyCreatesInlineThread() {
    ConversationBlip blip = target.getRootThread().appendBlip();
    MutableDocument<?, ?, ?> doc = blip.getDocument();
    ...
    doc.with(
        new Action() {
          ...
          public <N, E extends N, T extends N> void exec(MutableDocument<N, E, T> doc) {
            ...
            List<LocatedReplyThread<ConversationThread>> expected = new ArrayList<>();
  ...
  public void testDeleteConversationInvalidatesBlips() {
    ...
    verify(convListener).onBlipRemoved(blip1, null);
    verify(convListener).onBlipRemoved(blip2, null);
  }
  ...
  public void testDeleteConversationInvalidatesNonRootThreads() {
    ...
    ObservableConversationThread inlineThread =
      outerBlip.addReplyThread(locateAfterLineElement(outerBlip.getDocument()));
  ...
  public void testDeleteConversationEvents() {
    ...
    assertBlipInvalid(innerBlip);
    assertThreadInvalid(inlineThread);
    verify(convListener).onBeforeBlipRemoved(innerBlip, null);
    verify(convListener).onBlipRemoved(innerBlip, null);
    verify(convListener).onBeforeReplyRemoved(inlineThread, null);
    verify(convListener).onReplyRemoved(inlineThread, null);
    verify(convListener).onBeforeBlipRemoved(outerBlip, null);
    verify(convListener).onBlipRemoved(outerBlip, null);
  ...
  public void testDeleteInlineReplyDeletesAnchor() {
    ConversationBlip blip = target.getRootThread().appendBlip();
    XmlStringBuilder xmlBefore = XmlStringBuilder.innerXml(blip.getDocument());
    ConversationThread inlineReply = blip.addReplyThread(locateAfterLineElement(
        blip.getDocument()));
    ConversationBlip inlineReplyBlip = inlineReply.appendBlip();
    ...
    assertBlipInvalid(inlineReplyBlip);
    assertThreadInvalid(inlineReply);
    assertStructureEquivalent(xmlBefore, blip.getDocument());
  }
  ...
  public void testSetAnchorEventsAreFired() {
    ...
    target.addAnchorListener(listener);
    ...
    // Remove listener.
    target.removeAnchorListener(listener);
    target.setAnchor(anchor1);
...
//
//    verify(convListener).onParticipantAdded(p1, null);
//    verify(convListener).onParticipantAdded(p2, null);
//
//    target.addParticipant(p1);
//    verifyNoMoreInteractions(convListener);
//
//    target.removeParticipant(p2);
//    verify(convListener).onParticipantRemoved(p2, null);
  }
  ...
  public void testThreadAppendInsertBlipFiresEvent() {
    ...
    verify(convListener).onBlipAdded(b1mirror, null);

    target.getRootThread().insertBlip(b1, true);
    ObservableConversationBlip b2mirror = mirror.getRootThread().getFirstBlip();
    verify(convListener).onBlipAdded(b2mirror, null);
  ...
  public void testThreadRemovalFiresEvent() {
    ...
    t1.delete();
    verify(convListener).onBeforeBlipRemoved(b3mirror, null);
    verify(convListener).onBlipRemoved(b3mirror, null);
    verify(convListener).onBeforeReplyRemoved(t1mirror, null);
    verify(convListener).onReplyRemoved(t1mirror, null);
  ...
  public void testRootThreadRemovalDoesntFireEvent() {
    ...
    verify(convListener).onBeforeBlipRemoved(b1mirror, null);
    verify(convListener).onBlipRemoved(b1mirror, null);
  ...
  public void testBlipAppendReplyFiresEvent() {
    ...
    verify(convListener).onReplyAdded(t1mirror, null);
  ...
  public void testCompoundEventsFireBottomUp() {
    ...
    order.verify(convListener).onBeforeBlipRemoved(b2mirror, null);
    order.verify(convListener).onBlipRemoved(b2mirror, null);
    order.verify(convListener).onBeforeReplyRemoved(t1mirror, null);
    order.verify(convListener).onReplyRemoved(t1mirror, null);
    order.verify(convListener).onBeforeBlipRemoved(b1mirror, null);
    order.verify(convListener).onBlipRemoved(b1mirror, null);
  ...
  private static void assertThreadChildrenConsistent(ConversationBlip blip) {
    Set<ConversationThread> allChildren = new HashSet<>();
