...
public class DocumentBasedManifestThreadTest extends TestCase {
  ...
  public void testWiringUp() {
    ...
    final WaveletOperationContext opContext = null;
    final ObservableManifestBlip previous = null;
    ...
    blipsListener.getValue().onValueAdded(blip, previous, opContext);
    blipsListener.getValue().onValueRemoved(blip, opContext);

    verify(threadListener).onBlipAdded(same(blip), same(previous), same(opContext));
    verify(threadListener).onBlipRemoved(same(blip), same(opContext));
