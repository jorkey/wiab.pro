...
public class WaveletBasedConversationTest extends ConversationTestBase {
  ...
  public void testAbstractBlipIdMatchesConcreteBlipId() {
    ...
    assertEquals(convBlip.getId(), convBlip.getId());
  }
  ...
  public void testConversationBlipMetadataMatchesWavelet() {
    ...
    assertEquals(blip.getLastModifiedVersion(), convBlip.getLastModifiedVersion());
    assertEquals(blip.getLastModifiedTime(), convBlip.getLastModifiedTime());
  ...
  public void testAppendInlineReplyUpdatesManifest() {
    ...
    assertEquals(Blips.INITIAL_HEAD +
        ...
        XmlStringBuilder.innerXml(blip.getDocument()).toString());
    assertMirrorConversationEquivalent();
  }

//  public void testDeleteBlipNoRepliesUpdatesManifest() {
//    WaveletBasedConversationBlip blip = target.getRootThread().appendBlip();
//    blip.delete(null, DocOp.Direction.CONV_2_DOC);
//    assertManifestXml("");
...
//    WaveletBasedConversationBlip blip = target.getRootThread().appendBlip();
//    WaveletBasedConversationThread reply = blip.addInlineReplyThread(
...
//  }
  ...
  /**
   * Test that producing a complex conversation structure programmatically
  ...
  public void testComplexManifestProducesEquivalentConversation() {
    ...
    firstBlip.getDocument().with(new Action() {
      @Override
      public <N, E extends N, T extends N> void exec(MutableDocument<N, E, T> doc) {
  ...
  public void testConcurrentDeletionOfFinalBlipsLeavesEmptyThread() {
    WaveletBasedConversationBlip first = target.getRootThread().appendBlip();
    ...
    verify(threadListener).onBeforeBlipRemoved(b2, null);
    verify(threadListener).onBlipRemoved(b2, null);
    verify(convListener).onBeforeBlipRemoved(b2, null);
    verify(convListener).onBlipRemoved(b2, null);
    ...
    assertThreadValid(replyThread);
    assertEquals(Arrays.asList(replyThread), CollectionUtils.newArrayList(first.getReplyThreads()));
    verify(convListener).onBlipAdded(second, null);
  ...
  public void testConcurrentDeletionOfFinalThreadsLeavesEmptyBlip() {
    ...
    verify(threadListener).onBeforeBlipRemoved(t2b, null);
    verify(threadListener).onBlipRemoved(t2b, null);
    verify(convListener).onBeforeBlipRemoved(t2b, null);
    verify(convListener).onBlipRemoved(t2b, null);
    verify(blipListener).onBeforeReplyRemoved(t2, null);
    verify(blipListener).onReplyRemoved(t2, null);
    verify(convListener).onBeforeReplyRemoved(t2, null);
    verify(convListener).onReplyRemoved(t2, null);
