...
public class DocumentBasedManifestBlipTest extends TestCase {
  ...
  public void testWiringUp() {
    ...
    threadsListener.getValue().onValueAdded(thread, null, null);
    threadsListener.getValue().onValueRemoved(thread, null);

    // Pretends events from xml should cause emit of events on blip listener.
    WaveletOperationContext opContext = null;
    ObservableManifestThread previous = null;
    verify(blipListener).onReplyAdded(same(thread), same(previous), same(opContext));
    verify(blipListener).onReplyRemoved(same(thread), same(opContext));
    ...
    threadsListener.getValue().onValueAdded(thread, previous, opContext);
