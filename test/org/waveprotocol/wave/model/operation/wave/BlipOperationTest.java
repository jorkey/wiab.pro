...
public class BlipOperationTest extends OperationTestBase {
  ...
  static class FakeBlipOperation extends BlipOperation {
    ...
    public List<? extends BlipOperation> applyAndReturnReverse(BlipData target) {
      WaveletOperationContext reverseContext = createReverseContext(target, target.getLastModifiedVersion());
      doUpdate(target);
      doApply(target);
  ...
  private BlipData createBlipData() {
    return waveletData.createBlip("blipid", fred, noParticipants,
        ModelTestUtils.createContent(""), 0L, 0L, 0L, 0L);
  }
}
