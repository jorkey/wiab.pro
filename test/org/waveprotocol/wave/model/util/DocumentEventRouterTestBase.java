...
public abstract class DocumentEventRouterTestBase extends TestCase {
  ...
  public void testSelfRemoval() {
    ...
    class SelfRemovalListener implements ElementListener<Doc.E> {
      ...
      @Override
      public void onElementAdded(Doc.E element, WaveletOperationContext opContext) {
        fail();
      }
      
      @Override
      public void onElementRemoved(Doc.E element, WaveletOperationContext opContext) {
        removeCallCount++;
  ...
  public void testChildListeners() {
    ...
    verify(elmListenerOne).onElementAdded(childOne, null);
    WaveletOperationContext opContext = null;
    verify(elmListenerTwo, never()).onElementAdded(anyElement(), same(opContext));
    Doc.E childTwo = realDoc.createChildElement(elmTwo, "y", noAttribs());
    verify(elmListenerOne).onElementAdded(childOne, null);
    verify(elmListenerTwo).onElementAdded(childTwo, null);
    realDoc.deleteNode(childTwo);
    verify(elmListenerOne, never()).onElementRemoved(anyElement(), same(opContext));
    verify(elmListenerTwo).onElementRemoved(childTwo, null);
    realDoc.deleteNode(childOne);
    verify(elmListenerOne).onElementRemoved(childOne, null);
    verify(elmListenerTwo).onElementRemoved(childTwo, null);
  ...
  public void testRecursiveDeletion() {
    ...
    WaveletOperationContext opContext = null;    
    verify(childRemovedListener).onElementRemoved(same(child), same(opContext));
  ...
  public void testRecursiveInsertion() {
    ...
    doAnswer(
        ...
        }).when(parentListener).onElementAdded(anyElement(), same(opContext));
    ...
    verify(parentListener).onElementAdded(child, null);
