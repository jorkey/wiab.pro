...
public final class SupplementedWaveImplTest extends TestCase {
  ...
  public void testMarkBlipIsIdempotent() {
    ...
    long waveletVersion = w.getVersion();
    ...
    assertEquals(blipReadVersion, newBlipReadVersion);
  }
  ...
  public void testUnmutedSomeArchivedIsInbox() {
    substrate.archiveAtVersion(ROOT, ROOT_VERSION);
    assertIsInbox();
  }
  ...
  public void testUnmutedRemoteArchivedIsNotInbox() {
    substrate.archiveAtVersion(ROOT, ROOT_VERSION);
    substrate.archiveAtVersion(W1, W1_VERSION);
    assertIsArchived();
  }
  ...
  public void testParticipantReadState() {
    WaveletBasedConversation c = setUpWithWaveModel();
    WaveletId wId = c.getWavelet().getId();

    assertFalse(supplement.haveParticipantsEverBeenRead(wId));
    assertTrue(supplement.isParticipantsUnread(wId));

    supplement.markParticipantsAsRead(wId);
    assertTrue(supplement.haveParticipantsEverBeenRead(wId));
    assertFalse(supplement.isParticipantsUnread(wId));
  }
  ...
  public void testParticipantReadStateWithBlip() {
    ...
    supplement.markParticipantsAsRead(c.getWavelet().getId());
  }
  ...
  public void testReportsPendingNotification() {
    substrate.setNotifiedVersion(W1, W1_VERSION);
  ...
  public void testSeeingClearsPendingNotification() {
    ...
    when(primitiveSupplement.getNotifiedVersion(waveletId1)).thenReturn(VERSION_1);
    when(primitiveSupplement.getNotifiedVersion(waveletId2)).thenReturn(VERSION_2);
    SupplementedWaveImpl supplementedWave = new SupplementedWaveImpl(
        primitiveSupplement, supplementedView, DefaultFollow.ALWAYS);

    supplementedWave.markAsNotified();

    verify(primitiveSupplement).setNotifiedVersion(eq(waveletId1), eq(VERSION_1));
    verify(primitiveSupplement).setNotifiedVersion(eq(waveletId2), eq(VERSION_2));
