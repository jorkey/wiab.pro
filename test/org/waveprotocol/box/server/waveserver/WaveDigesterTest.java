...
public class WaveDigesterTest extends TestCase {
  ...
  public void testWaveletWithNoBlipsResultsInEmptyTitleAndNoBlips() {
    ...
    WaveDigest digest = digester.generateDigest(conversation, observableWaveletData);
  ...
  public void testUnreadCount() {
    ...
    Map<ParticipantId, WaveDigestSupplement> digestSupplements =
        digester.generateDigestSupplements(ImmutableSet.of(PARTICIPANT),
        conversation, ImmutableMap.of(PARTICIPANT, supplement));

    assertEquals(3, digest.getBlipCount());
    assertEquals(2, digestSupplements.get(PARTICIPANT).getUnreadCount());
  }
}
