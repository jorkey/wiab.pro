...
public class EventGeneratorTest extends RobotsTestBase {
  ...
  private final WaveletOperationContext.Factory CONTEXT_FACTORY =
      new WaveletOperationContext.Factory() {
        @Override
        public WaveletOperationContext createContext() {
          return new WaveletOperationContext(ALEX, 0L, wavelet.getVersion()+1,
            HashedVersion.unsigned(wavelet.getVersion()+1));
        }
  ...
  protected void setUp() throws Exception {
    ...
    waveletData.addParticipant(ROBOT, null);
    waveletData.addParticipant(ALEX, null);
    waveletData.setHashedVersion(HashedVersion.unsigned(1));
  ...
  public void testGenerateWaveletSelfAddedEvent() throws Exception {
    waveletData.removeParticipant(ROBOT, null);
  ...
  public void testRobotSelfEventsFilteredBeforeHuman() throws Exception {
    ...
    LineContainers.appendToLastLine(rootBlip.getDocument(), builder);
    // Delta1 event #3.
    wavelet.addParticipant(BOB);
  ...
  public void testGenerateWaveletBlipRemovedEvent() throws Exception {
    ...
    WaveletBlipRemovedEvent event = WaveletBlipRemovedEvent.as(messages.getEvents().get(2));
  ...
  public void testGenerateAnnotatedTextChangedEvent() throws Exception {
    ...
    rootBlip.getDocument().setAnnotation(0, 1, annotationKey, annotationValue);

    EventMessageBundle messages = generateAndCheckEvents(EventType.ANNOTATED_TEXT_CHANGED);
    assertEquals("Expected two events only", 2, messages.getEvents().size());
