...
public class DocumentModifyServiceTest extends RobotsTestBase {
  ...
  protected void setUp() throws Exception {
    ...
    service = new DocumentModifyService(attachmentService, idGenerator);
    ...
    LineContainers.appendToLastLine(
        rootBlip.getDocument(), XmlStringBuilder.createText(INITIAL_CONTENT));
  }
  ...
  public void testAnnotate() throws Exception {
    ...
    String annotation = getRootBlip().getDocument().getAnnotation(CONTENT_START_XML, ANNOTATION_KEY);
    ...
    assertNull("Expected this text not to be annotated",
        getRootBlip().getDocument().getAnnotation(CONTENT_START_XML + 1, ANNOTATION_KEY));
  }

  public void testClearAnnotatation() throws Exception {
    Document doc = getRootBlip().getDocument();
    ...
    assertNull("Expected this text not to be annotated",
        getRootBlip().getDocument().getAnnotation(CONTENT_START_XML, ANNOTATION_KEY));
  }
  ...
  public void testUpdateGadget() throws Exception {
    ...
    ObservableConversationBlip rootBlip = getRootBlip();
    rootBlipId = rootBlip.getId();
    LineContainers.appendToLastLine(rootBlip.getDocument(),
  ...
  private ApiView getApiView() throws InvalidRequestException {
    ApiView view = new ApiView(
        getRootBlip().getDocument(), helper.getContext().openWavelet(WAVE_ID, WAVELET_ID, ALEX));
    return view;
  }
