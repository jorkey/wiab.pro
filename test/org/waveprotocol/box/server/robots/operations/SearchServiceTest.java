...
public class SearchServiceTest extends TestCase {
  ...
  public void testSearchWrapsSearchProvidersResult() throws InvalidRequestException {
    String title = "title";
    WaveDigest waveDigest = new WaveDigest(WAVE_ID.serialise(), title, "", PARTICIPANT.getAddress(),
        ImmutableList.of(PARTICIPANT.getAddress(), OTHER_PARTICIPANT.getAddress()), 1, -1L, -1L);
    WaveDigestSupplement supplement = new WaveDigestSupplement(WaveDigestSupplement.FOLDER_INBOX, 1);
    SearchResult.Digest digest = new SearchResult.Digest(waveDigest, supplement);
    String query = "in:inbox";
    SearchResult searchResult = new SearchResult(query);
    searchResult.addDigest(digest);
    when(searchProvider.search(query, 0, 10, USER)).thenReturn(searchResult);
  ...
  public void testResultsAreWhatTheSearchProviderSaysIncludingEmptyWaves() throws Exception {
    ...
    String query = "in:inbox";
    WaveDigest waveDigest = new WaveDigest(WAVE_ID.serialise(), "", "", PARTICIPANT.getAddress(),
        ImmutableList.of(PARTICIPANT.getAddress()), 1, -1L, -1L);
    WaveDigestSupplement supplement = new WaveDigestSupplement(WaveDigestSupplement.FOLDER_INBOX, 1);
    SearchResult.Digest digest = new SearchResult.Digest(waveDigest, supplement);
    SearchResult searchResult = new SearchResult(query);
    searchResult.addDigest(digest);
  ...
  public void testDefaultFieldsMatchSpec() throws InvalidRequestException {
    String query = "in:inbox";
    when(searchProvider.search(query, 0, 10, USER)).thenReturn(new SearchResult(query));
    service.execute(operation, context, USER);

    verify(searchProvider).search(query, 0, 10, USER);
  }
  ...
  public Matcher<Map<ParamsProperty, Object>> matchesSearchResult(final String query,
    ...
    return new BaseMatcher<Map<ParamsProperty, Object>>() {
      ...
      public boolean matches(Object item) {
        ...
        SearchResult.Digest digest = results.getDigests().get(0);
