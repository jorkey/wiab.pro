...
public abstract class DeltaStoreTestBase extends TestCase {
  ...
  public void testOpenNonexistantWavelet() throws Exception {
    ...
    DeltaAccess wavelet = store.open(WAVE1_WAVELET1);
    ...
    assertNull(wavelet.getLastModifiedVersion());
    assertNull(wavelet.getDeltaByStartVersion(0));
  ...
  public void testWriteToNewWavelet() throws Exception {
    ...
    assertEquals(WAVE1_WAVELET1, wavelet.getWaveletName());
    assertEquals(record.getResultingVersion(), wavelet.getLastModifiedVersion());
    assertEquals(record, wavelet.getDeltaByStartVersion(0));
    assertEquals(record, wavelet.getDeltaByEndVersion(record.getResultingVersion().getVersion()));
    assertEquals(record.getAppliedAtVersion(), wavelet.getDeltaByStartVersion(0).getAppliedAtVersion());
    assertEquals(record.getAppliedDelta(), wavelet.getDeltaByStartVersion(0).getAppliedDelta());
    assertEquals(record.getTransformedDelta(), wavelet.getDeltaByStartVersion(0).getTransformedDelta());
  ...
  public void testWaveIdIteratorLimits() throws Exception {
    Pair<DeltaStore,WaveletDeltaRecord> pair = newDeltaStoreWithRecord(WAVE1_WAVELET1);
    DeltaStore store = pair.first;
    DeltaAccess wavelet = store.open(WAVE2_WAVELET1);
  ...
  protected WaveletDeltaRecord createRecord() {
    ...
    List<WaveletOperation> ops =
        ImmutableList.of((WaveletOperation)UTIL.noOp(), UTIL.addParticipant(TestingConstants.OTHER_PARTICIPANT));
    ...
    ProtocolWaveletDelta serializedDelta = OperationSerializer.serialize(transformed);
    ...
    ProtocolAppliedWaveletDelta delta =
        ProtocolAppliedWaveletDelta.newBuilder().setApplicationTimestamp(1234567890)
            .setHashedVersionAppliedAt(OperationSerializer.serialize(targetVersion))
  ...
  private Pair<DeltaStore, WaveletDeltaRecord> newDeltaStoreWithRecord(WaveletName waveletName)
      throws Exception {
    DeltaStore store = newDeltaStore();
    DeltaAccess wavelet = store.open(waveletName);
